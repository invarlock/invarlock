...
def overlapping_project():
    ...
    # Just 177 pairs can exceed the measured work envelope when every one of
...
def test_omitted_budget_rejects_before_scoring(check, monkeypatch):
    ...
    with pytest.raises(PipelineError, match="local budget is 98304000"):
...
def test_explicit_none_preserves_complete_replay_but_not_recipient_default(monkeypatch):
    ...
    assert all(m["count"] == 177 for m in replay["metrics"])
...
def test_cli_omitted_budget_rejects_without_publishing(tmp_path):
    ...
    assert "local budget is 98304000" in json.loads(result.stdout)["message"]
...
def test_cli_recipient_requires_its_own_larger_budget(tmp_path):
    ...
    assert "local budget is 98304000" in json.loads(rejected.stdout)["message"]
    accepted = runner.invoke(
        app, [*args, "--max-bootstrap-draws", str(177 * 16 * 17 * 2048)]
    )
