...
def module(name):
    spec = importlib.util.spec_from_file_location(
        name, ROOT / f"examples/captured-results/{name}.py"
    )
...
def test_native_projection_keeps_failed_quality_as_measurable_results(bundle, tmp_path):
    ...
    for kind in ("classification", "extraction", "numeric"):
        ...
        from invarlock.engine import evaluate_request_file

        evaluated = evaluate_request_file(
            directory / "request.yaml", signing_key_path=None, unsigned=True
        )
        assert evaluated.policy_verdict == result["decision"]
        assert (
            json.loads(
                (
                    evaluated.evidence_path / "reports/evaluation.report.json"
                ).read_bytes()
            )
            == result
        )
