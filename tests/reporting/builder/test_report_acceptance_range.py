import pytest

from invarlock.reporting import report_policy as policy


def test_resolve_pm_acceptance_range_paths(monkeypatch):
    # No explicit bounds -> empty payload
    monkeypatch.delenv("INVARLOCK_PM_ACCEPTANCE_MIN", raising=False)
    monkeypatch.delenv("INVARLOCK_PM_ACCEPTANCE_MAX", raising=False)
    assert policy.resolve_pm_acceptance_range_from_report({}) == {}

    # Context-provided bounds with missing min should use default minimum.
    report_ctx = {
        "context": {
            "primary_metric": {"acceptance_range": {"min": None, "max": "1.3"}},
        }
    }
    adjusted = policy.resolve_pm_acceptance_range_from_report(report_ctx)
    assert adjusted["min"] == pytest.approx(0.95)
    assert adjusted["max"] == pytest.approx(1.3)

    # Explicit context bounds without env overrides should pass through
    monkeypatch.delenv("INVARLOCK_PM_ACCEPTANCE_MIN", raising=False)
    monkeypatch.delenv("INVARLOCK_PM_ACCEPTANCE_MAX", raising=False)
    passthrough = policy.resolve_pm_acceptance_range_from_report(
        {
            "context": {
                "primary_metric": {"acceptance_range": {"min": 0.97, "max": 1.02}}
            }
        }
    )
    assert passthrough == {"min": 0.97, "max": 1.02}

    # Invalid ordering from explicit context data is clamped for monotonicity.
    clamped = policy.resolve_pm_acceptance_range_from_report(
        {"context": {"primary_metric": {"acceptance_range": {"min": 1.2, "max": 1.0}}}}
    )
    assert clamped == {"min": 1.2, "max": 1.2}

    meta_fallback = policy.resolve_pm_acceptance_range_from_report(
        {"meta": {"pm_acceptance_range": {"min": 0.96, "max": 1.08}}}
    )
    assert meta_fallback == {"min": 0.96, "max": 1.08}
