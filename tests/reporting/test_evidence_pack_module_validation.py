...
import invarlock.evidence_pack_manifest as evidence_pack_manifest_mod
...
def test_evidence_pack_manifest_jsonschema_helper_uses_exceptions_and_fallback_attrs(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(evidence_pack_manifest_mod, "jsonschema", None, raising=False)
    assert evidence_pack_manifest_mod._jsonschema_validation_error_types() == ()
    ...
    monkeypatch.setattr(
        evidence_pack_manifest_mod, "jsonschema", jsonschema_stub, raising=False
    )
...
def test_evidence_pack_manifest_validate_manifest_covers_schema_failure_and_direct_validate(
    monkeypatch, tmp_path: Path
) -> None:
    manifest_path = tmp_path / "manifest.json"
    manifest_path.write_text("{invalid", encoding="utf-8")
    errors = evidence_pack_manifest_mod.validate_manifest(manifest_path)
    assert "manifest is not valid JSON" in errors[0]

    _write_json(
        manifest_path,
        {
            "format": evidence_pack_manifest_mod.EVIDENCE_PACK_FORMAT,
            "checksums_sha256": "checksums.sha256",
            "checksums_sha256_digest": "a" * 64,
        },
    )
    monkeypatch.setattr(
        evidence_pack_manifest_mod,
    ...
    assert evidence_pack_manifest_mod.validate_manifest(manifest_path) == []
    assert calls == [
        (
...
def test_evidence_pack_validate_manifest_falls_back_to_manual_checks_without_schema(
    ...
    manifest_errors = evidence_pack_manifest_mod.validate_manifest(manifest_path)
