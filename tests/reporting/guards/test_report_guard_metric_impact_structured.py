...
def _mk_pm_report(*, ratio: float, pm_final: float = 10.0) -> dict:
    return {
        ...
        "evaluation_windows": {
            "final": {
                "window_ids": [1],
                "logloss": [math.log(pm_final * ratio)],
                "token_counts": [1],
            }
        },
    }
...
def test_guard_metric_impact_structured_pass_and_fail() -> None:
    # Prepare bare/guarded reports to exercise structured validate_guard_metric_impact path
    bare = _mk_pm_report(ratio=1.0, pm_final=10.0)
    guarded = _mk_pm_report(ratio=1.01, pm_final=10.0)  # 1% degradation

    # PASS: the 2% limit is above the 1% relative degradation.
    rep_pass = _mk_pm_report(ratio=1.0, pm_final=10.0)
    rep_pass["guard_metric_impact"] = {
        "degradation_limit": 0.02,
        "bare_report": bare,
        "guarded_report": guarded,
    }
    base = _mk_pm_report(ratio=1.0, pm_final=10.0)
    base["edit"]["name"] = "noop"
    base["metrics"]["primary_metric"].pop("ratio_vs_baseline")
    cert_pass = make_report(rep_pass, base)
    go_pass = cert_pass.get("guard_metric_impact", {})
    ...
    assert "Guard Metric Impact Acceptable" in labels
    # Guard row status should be PASS
    row = next(
        r for r in block_pass["rows"] if r["label"] == "Guard Metric Impact Acceptable"
    )
    assert row["ok"] is True and row["status"].startswith("✅")

    # FAIL: the 0.5% limit is below the 1% relative degradation.
    rep_fail = _mk_pm_report(ratio=1.0, pm_final=10.0)
    rep_fail["guard_metric_impact"] = {
        "degradation_limit": 0.005,
        "bare_report": bare,
        "guarded_report": guarded,
    }
    cert_fail = make_report(rep_fail, base)
    go_fail = cert_fail.get("guard_metric_impact", {})
    ...
    row2 = next(
        r for r in block_fail["rows"] if r["label"] == "Guard Metric Impact Acceptable"
    )
