...
from invarlock import proof_pack_metadata as mod


def test_proof_pack_counts_from_verification_extracts_only_integer_counts() -> None:
    assert mod._proof_pack_counts_from_verification(None) == (None, None, None)
    assert mod._proof_pack_counts_from_verification(
        {
            "clean_reports": 2,
...
def test_derive_proof_pack_evidence_level_requires_full_high_evidence_set() -> None:
    assert (
        mod._derive_proof_pack_evidence_level(
...
def test_render_proof_pack_readme_covers_failed_and_strict_variants() -> None:
    rendered = mod._render_proof_pack_readme(
        evidence_level="high",
        clean_reports=4,
        error_reports=1,
        failed_reports=2,
...
def test_render_proof_pack_readme_covers_default_guidance_without_failures() -> None:
    rendered = mod._render_proof_pack_readme(
        evidence_level="low",
        clean_reports=None,
        error_reports=None,
