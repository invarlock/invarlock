...
from scripts.proof_packs.python import preset_generator, runtime_tools
...
@pytest.mark.parametrize(
    "relative_path",
    [
        "scripts/proof_packs/python/rmt_cross_model_probe.py",
        "scripts/proof_packs/python/ve_cross_model_probe.py",
    ],
)
...
def test_proof_pack_shell_flows_keep_attestation_enforced() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    run_pack = (repo_root / "scripts/proof_packs/run_pack.sh").read_text(
        encoding="utf-8"
    )
    verify_pack = (repo_root / "scripts/proof_packs/verify_pack.sh").read_text(
        encoding="utf-8"
    )
    task_functions = (
