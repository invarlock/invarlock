...
def test_dependency_manager_missing_modules(monkeypatch):
    ...
    monkeypatch.setattr(support_mod.importlib, "import_module", unavailable)

    dep_manager = DependencyManager()
    assert dep_manager.get_missing_dependencies()[0][0] == "scikit-learn"
    with pytest.raises(DependencyError):
        dep_manager.get_module("mi_scores")
    assert dep_manager.is_available("scan_model_gains")
...
def test_dependency_manager_available_modules(monkeypatch):
    ...
    monkeypatch.setattr(
        support_mod.importlib, "import_module", lambda _name: FeatureSelection()
    )

    dep_manager = DependencyManager()
    assert dep_manager.is_available("mi_scores")
    assert dep_manager.get_module("scan_model_gains") is support_mod.scan_model_gains
...
def test_gini_vectorized_and_mi_gini_cpu_path():
    ...
    targ = torch.rand(5)
    config = MetricsConfig(use_cache=False)
    result = _mi_gini_optimized_cpu_path(
        feats,
        targ,
        max_per_layer=10,
        config=config,
        mi_scores_fn=lambda x, _y: x.mean(dim=0),
    )
