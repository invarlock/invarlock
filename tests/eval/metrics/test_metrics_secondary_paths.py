...
def test_get_metrics_info_and_validate_env():
    ...
    assert {"sigma_max", "head_energy"}.issubset(info["available_metrics"])
    mi_available = "mi_gini" in info["available_metrics"]
    assert mi_available == ("scikit-learn" in info["available_dependencies"])
    assert ("mi_gini" in info["unavailable_metrics"]) is not mi_available
    report = ME.validate_metrics_environment()
    assert report.ok is True
    assert ("scikit-learn" in report.available_dependencies) is mi_available
...
def test_mi_gini_cpu_chunk_warning(monkeypatch):
    ...
    cfg = M.MetricsConfig()
    out = _mi_gini_optimized_cpu_path(
        feats,
        targ,
        max_per_layer=10,
        config=cfg,
