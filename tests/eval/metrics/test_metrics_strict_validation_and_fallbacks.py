...
def test_mi_gini_optimized_cpu_path_subsamples_when_n_gt_max(monkeypatch) -> None:
    ...
    monkeypatch.setattr(activation_mod.torch, "randperm", lambda n: torch.arange(n))
...
def test_locate_transformer_blocks_enhanced_catches_len_typeerror_and_uses_fallback() -> (
    ...
    blocks = activation_mod._locate_transformer_blocks_enhanced(_Model())
...
def test_calculate_sigma_max_rejects_invalid_scan_result() -> None:
    class _DepMgr:
        def is_available(self, _name: str) -> bool:  # noqa: ANN001
            return True

        def get_module(self, _name: str):  # noqa: ANN001
            return lambda _model: object()
...
def test_calculate_head_energy_returns_nan_when_all_values_non_finite() -> None:
    ...
    out = activation_mod._calculate_head_energy(hidden_states_list, cfg)
    assert math.isnan(out)
...
def test_calculate_mi_gini_returns_nan_for_missing_dependency_and_missing_activations() -> (
    ...
    out_missing = activation_mod._calculate_mi_gini(
        nn.Linear(2, 2),
    ...
    out_empty = activation_mod._calculate_mi_gini(
        nn.Linear(2, 2),
...
def test_calculate_sigma_max_all_non_finite_triggers_nan_path() -> None:
    class _DepMgr:
        ...
        def get_module(self, _name: str):  # noqa: ANN001
            def _scan(_model):  # noqa: ANN001
                return {"spectral_norms": [float("nan")]}
...
def test_calculate_sigma_max_uses_canonical_spectral_norms() -> None:
    class _DepMgr:
        def is_available(self, _name: str) -> bool:  # noqa: ANN001
            return True
        ...
        def get_module(self, _name: str):  # noqa: ANN001
            def _scan(_model):  # noqa: ANN001
                return {"spectral_norms": [0.5, 1.25], "scanned_modules": 2}
...
def test_calculate_sigma_max_ignores_nonfinite_values_when_finite_values_exist() -> (
    None
):
    class _DepMgr:
        def is_available(self, _name: str) -> bool:  # noqa: ANN001
        ...
        def get_module(self, _name: str):  # noqa: ANN001
            return lambda _model: {"spectral_norms": [float("nan"), 1.75, float("inf")]}
...
def test_calculate_mi_gini_cpu_scoring_does_not_touch_cuda_cache(
    monkeypatch,
) -> None:
    called = {"empty_cache": 0}
    monkeypatch.setattr(activation_mod.torch.cuda, "is_available", lambda: True)
    monkeypatch.setattr(
        activation_mod.torch.cuda,
        "empty_cache",
        lambda: called.__setitem__("empty_cache", called["empty_cache"] + 1),
    )
    monkeypatch.setattr(
        activation_mod, "_mi_gini_optimized_cpu_path", lambda *_a, **_k: float("nan")
    )
    ...
    class _DepMgr:
        ...
        def get_module(self, _name: str):  # noqa: ANN001
            def _failure(_feats, _targ):  # noqa: ANN001
    ...
    assert math.isnan(
        activation_mod._calculate_mi_gini(
            nn.Linear(2, 2),
            activation_data=activation_data,
            dep_manager=_DepMgr(),
            config=cfg,
    ...
    assert called["empty_cache"] == 0


def test_calculate_mi_gini_layer_error_returns_nan() -> None:
    class _DepMgr:
        def is_available(self, _name: str) -> bool:  # noqa: ANN001
            return True
...
def test_validate_metrics_environment_reports_missing_modules(monkeypatch) -> None:
    class _DepMgr:
        def __init__(self):  # noqa: D401
            self.available_modules = {"scan_model_gains": object()}
            self.missing_modules = [("scikit-learn", "boom")]
    ...
    assert report.missing_dependencies == (("scikit-learn", "boom"),)
    assert "scikit-learn: boom" in report.messages
...
def test_validate_metrics_environment_reports_clean_optional_dependency_set(
    ...
    class _DepMgr:
        available_modules = {"scan_model_gains": object(), "mi_scores": object()}
        missing_modules = []
    ...
    assert report.ok is True
    assert report.available_dependencies == ("scikit-learn",)
...
def test_forward_loss_causal_handles_models_without_return_dict() -> None:
    ...
    loss, logits = model_io_mod.forward_loss_causal(
        _Model(),
