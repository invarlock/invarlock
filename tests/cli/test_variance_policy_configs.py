...
def test_balanced_tier_doc_matches_config():
    ...
    assert doc["min_effect_lognll"] == pytest.approx(0.0009)
...
def test_conservative_tier_doc_matches_config():
    ...
    assert doc["min_effect_lognll"] == pytest.approx(0.0018)
...
def test_balanced_spectral_doc_knobs():
    ...
    # Calibrated values from spectral sweep (2025-12)
    assert doc["family_caps"]["ffn"] == pytest.approx(3.834)
...
def test_conservative_spectral_doc_knobs():
    ...
    assert doc["family_caps"]["ffn"] == pytest.approx(2.3)
...
def test_balanced_rmt_doc_knobs():
    ...
    assert doc["epsilon_default"] == pytest.approx(0.10)
    assert doc["epsilon_by_family"]["ffn"] == pytest.approx(0.10)
    assert doc["epsilon_by_family"]["attn"] == pytest.approx(0.08)
    assert doc["epsilon_by_family"]["embed"] == pytest.approx(0.12)
    assert doc["epsilon_by_family"]["other"] == pytest.approx(0.12)
...
def test_conservative_rmt_doc_knobs():
    ...
    assert doc["epsilon_default"] == pytest.approx(0.06)
    assert doc["epsilon_by_family"]["ffn"] == pytest.approx(0.06)
    assert doc["epsilon_by_family"]["attn"] == pytest.approx(0.05)
    assert doc["epsilon_by_family"]["embed"] == pytest.approx(0.07)
    assert doc["epsilon_by_family"]["other"] == pytest.approx(0.07)
