...
def _dump_plain_yaml(path: Path, payload: object) -> None:
    path.write_text(
        yaml.safe_dump(_plain_yaml_value(payload), sort_keys=False),
        encoding="utf-8",
    )
...
def test_evaluate_profile_str_failure_falls_back_to_non_ci(
    ...
    monkeypatch.setattr(mod, "_dump_yaml", _dump_plain_yaml, raising=False)
...
def test_evaluate_stable_text_uses_fallback_when_stringification_raises(
    ...
    base_run = _fake_run_command_with_paths(
        {"source": baseline_report, "edited": edited_report}
    )

    def run_with_fallback_profile(**kwargs):
        return base_run(**{**kwargs, "profile": "dev"})

    monkeypatch.setattr(
        run_mod,
        "run_command",
        run_with_fallback_profile,
        raising=False,
    )
