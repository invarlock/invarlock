...
def test_primary_metric_validation_ppl_and_non_ppl() -> None:
    ...
    # Explicit degraded outcomes must fail verification rather than pass silently.
    cert_invalid = {
        "primary_metric": {
            "kind": "ppl_causal",
            "final": float("nan"),
...
def test_primary_metric_validation_rejects_rollback_outcomes() -> None:
    verify_mod = _import_verify_module()
    metric = {
        "kind": "ppl_causal",
        "preview": 10.0,
    ...
    status_errors = verify_mod._validate_primary_metric(
        {
            "status": "rollback",
    ...
    assert any(
        "not a successful evaluation outcome" in error for error in status_errors
    )

    flag_errors = verify_mod._validate_primary_metric(
        {
            "flags": {"rollback_reason": "primary_metric_invalid"},
            "primary_metric": metric,
            "baseline_ref": {"primary_metric": {"final": 10.0}},
        }
    )
    assert any("records a rollback" in error for error in flag_errors)
