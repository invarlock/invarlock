...
def _build_sample_evaluation_report() -> dict:
    ...
    cert = {
        ...
        "evaluation_windows": {
            "final": {"logloss": [math.log(ppl_final)], "token_counts": [1]}
        },
        "policies": {
            "spectral": {
                "sigma_quantile": 0.95,
    ...
    cert.setdefault("dataset", {}).setdefault("windows", {}).setdefault(
    ...
    ).update(
        {
            ...
            "preview_final_slice_delta_summary": {
                "mean": logloss_delta,
                "ci": [logloss_delta - 0.01, logloss_delta + 0.01],
...
def test_verify_delta_basis_matches_pm_direction(tmp_path: Path):
    ...
    cert["baseline_ref"]["primary_metric"] = {
        "kind": "accuracy",
        "final": 0.91,
    }
    cert["metrics"] = {"classification": {"n_correct": 93, "n_total": 100}}
    # Verify processes the accuracy-specific baseline comparison field.
