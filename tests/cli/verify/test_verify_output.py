...
def test_build_recompute_summary_ppl_paths() -> None:
    ...
    assert verify_output._build_recompute_summary(
        ...
        tolerance=1e-9,
    ) == {"family": "ppl", "performed": True, "ok": True, "reason": None}
...
def test_build_verify_error_payload_and_success_line() -> None:
    ...
    accuracy_line = verify_output.build_verify_success_line(
        {
            "primary_metric": {
                "kind": "accuracy",
                "final": 0.8,
    ...
    assert accuracy_line == (
        "VERIFY OK metric=accuracy delta_vs_baseline_pp=5.000000 "
        "ci=[4.000000,6.000000] width=2.000000"
    )
