...
def test_verify_json_malformed_input_preempts_policy_checks(
    ...
    assert out["summary"] == {"ok": False, "reason": "malformed"}
    assert len(out["results"]) == 1
    assert out["results"][0]["id"] == str(p1)
    assert out["results"][0]["reason"] == "malformed"
    assert "error" in out
