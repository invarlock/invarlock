...
def _cert_base(pm: dict[str, Any]) -> dict[str, Any]:
    pm = dict(pm)
    pm.setdefault("preview", pm.get("final"))
    return {
        "schema_version": "v1",
...
def test_validate_primary_metric_ppl_baseline_final_numeric_string_is_coerced() -> None:
    cert = _cert_base({"kind": "ppl_causal", "final": 110.0, "ratio_vs_baseline": 1.1})
    cert["baseline_ref"]["primary_metric"]["final"] = "100"
    assert v._validate_primary_metric(cert) == []


def test_validate_primary_metric_accuracy_requires_delta_pp():
...
def test_validate_primary_metric_accuracy_rejects_ratio_field() -> None:
    cert = _cert_base(
        {
            "kind": "accuracy",
            "final": 0.9,
