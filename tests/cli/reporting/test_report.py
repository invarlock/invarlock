...
def test_run_report_health_helpers_cover_edge_cases():
    ...
    assert (
        _describe_run_report_health_error(
            {"status": "rollback", "metrics": {"primary_metric": {}}},
            role="subject",
        )
    ...
    assert (
        ...
        == "Cannot generate evaluation report from baseline run report with "
        "non-finite primary metric field 'ratio_vs_baseline'."
    )
...
@patch("invarlock.reporting.report_contract.save_evaluation_bundle")
@patch("invarlock.reporting.report_contract.validate_report")
def test_report_command_rejects_non_finite_input_ratio_without_pairing_reason(
    ...
    with pytest.raises(ValueError, match="non-finite primary metric field"):
        generate_reports(
            run="run.json",
            format="report",
            compare=None,
