...
import invarlock.cli.run_metric_impact as run_metric_impact_mod
import invarlock.eval.primary_metric as primary_metric_mod
from invarlock.cli.run_metric_impact import (
    _extract_pm_snapshot_for_metric_impact as extract,
)
...
def test_extract_pm_snapshot_continues_after_dict_compute_failure(
    ...
    monkeypatch.setattr(run_metric_impact_mod, "_compute_snapshot_from_report", _boom)
...
def test_validate_guard_metric_impact_with_extracted_pm_evaluates() -> None:
    ...
    res = validate_guard_metric_impact(
        bare_struct, guarded_struct, degradation_limit=0.5
    )
    assert hasattr(res, "metrics")
    ratio = res.metrics.get("degradation")
