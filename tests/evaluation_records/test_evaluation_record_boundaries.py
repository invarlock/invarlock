...
from tests._evaluation_support import compare_runs, make_run
...
def test_recorded_scores_require_explicit_provenance_policy():
    ...
    with pytest.raises(EvaluationRecordsError, match="provenance"):
...
@pytest.mark.parametrize("scope", ["policy", "metric"])
def test_unknown_policy_fields_cannot_be_ignored(scope):
    selected = policy()
    field = "unexpected" if scope == "policy" else "minimun_count"
    target = selected if scope == "policy" else selected["metrics"][0]
    target[field] = 999
