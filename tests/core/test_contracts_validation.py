...
from invarlock.guards import rmt_analysis as rmt_contracts
from invarlock.guards import spectral_control as spectral_contracts


def test_contracts_basic_paths():
    W = torch.randn(4, 4)
    # enforce_relative_spectral_cap returns same type and caps if needed
    out = spectral_contracts.enforce_relative_spectral_cap(
        W.clone(), baseline_sigma=1.0, cap_ratio=0.5
    )
    assert isinstance(out, torch.Tensor)

    # rmt_correction_is_monotone true/false branches
    assert rmt_contracts.rmt_correction_is_monotone(
        1.0, baseline_sigma=2.0, max_ratio=5.0, deadband=0.1
    )
    assert not rmt_contracts.rmt_correction_is_monotone(
...
def test_enforce_relative_spectral_cap_noop_when_sigma_below_limit() -> None:
    W = torch.zeros(4, 4)
    out = spectral_contracts.enforce_relative_spectral_cap(
        W.clone(), baseline_sigma=10.0, cap_ratio=1.0
    )
...
def test_spectral_norm_reshape_and_cpu_fallback(monkeypatch) -> None:
    ...
    val = spectral_contracts._spectral_norm(W)
