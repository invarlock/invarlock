...
def test_provider_kwargs_propagate_top_level_revision_and_nested_override() -> None:
    top_level_revision = "a" * 40
    nested_revision = "b" * 40
    top_level = SimpleNamespace(
        dataset_name="public/dataset",
        revision=top_level_revision,
        provider="hf_text",
    )
    nested = SimpleNamespace(
        dataset_name="public/dataset",
        revision=top_level_revision,
        provider=_ProviderConfig("hf_text", revision=nested_revision),
    )

    assert _build_provider_kwargs(top_level) == {
        "dataset_name": "public/dataset",
        "revision": top_level_revision,
    }
    assert _build_provider_kwargs(nested) == {
        "dataset_name": "public/dataset",
