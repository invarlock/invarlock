...
def test_resolve_guard_metric_degradation_limit_and_tier_target_exceptions(
    monkeypatch,
) -> None:
    def _boom(_obj: object) -> dict[str, object]:
        raise RuntimeError("boom")
    ...
    with pytest.raises(RuntimeError, match="boom"):
        policy.resolve_guard_metric_degradation_limit(
            {"primary_metric": {"degradation_limit": 0.2}},
            coerce_mapping_fn=_boom,
        )
    with pytest.raises(RuntimeError, match="boom"):
        policy.resolve_guard_metric_degradation_limit(
...
def test_skip_policy_and_bool_coercion_edges() -> None:
    ...
    assert policy.resolve_skip_guard_metric_impact_policy(
    ...
    cfg = {"context": {"eval": {"skip_guard_metric_impact_check": "on"}}}
    assert policy.resolve_skip_guard_metric_impact_policy(cfg) == (
        True,
        "config:context.eval.skip_guard_metric_impact_check",
    )

    measure, skip, source = policy.should_measure_metric_impact("ci", cfg)
    ...
    run_cfg = {"context": {"run": {"skip_guard_metric_impact_check": "1"}}}
    assert policy.resolve_skip_guard_metric_impact_policy(run_cfg) == (
        True,
        "config:context.run.skip_guard_metric_impact_check",
    )
    assert policy.should_measure_metric_impact("dev", run_cfg) == (False, False, None)
...
def test_guard_metric_degradation_limit_and_dataset_split_helpers() -> None:
    assert (
        policy.resolve_guard_metric_degradation_limit(None, default_limit=0.02) == 0.02
    )
    assert (
        policy.resolve_guard_metric_degradation_limit(
            {"primary_metric": {"degradation_limit": 0.5}}
        )
        == 0.5
    )
...
def test_baseline_guard_evidence_accepts_mapping_and_list_reports() -> None:
    spectral = {"status": "pass", "modules": [{"name": "layer.0"}]}
    mapping_result = policy._baseline_guard_evidence(
        {
            "guards": {
                "Spectral": spectral,
    ...
    assert mapping_result == {
        "spectral": {
            "name": "Spectral",
            "status": "pass",
            "modules": [{"name": "layer.0"}],
        }
    }
    assert mapping_result["spectral"] is not spectral
    spectral["modules"][0]["name"] = "mutated"
    assert mapping_result["spectral"]["modules"][0]["name"] == "layer.0"

    list_result = policy._baseline_guard_evidence(
        {
            "guards": [
                {"name": "RMT", "epsilon": 0.1},
                {"name": "unknown", "value": 1},
                "not-a-mapping",
            ]
        }
    )
    assert list_result == {"rmt": {"name": "RMT", "epsilon": 0.1}}


@pytest.mark.parametrize(
    "report",
    [None, [], {}, {"guards": "invalid"}, {"guards": []}, {"guards": {}}],
)
def test_baseline_guard_evidence_rejects_missing_or_unusable_reports(report) -> None:
    assert policy._baseline_guard_evidence(report) is None
