...
def test_build_run_report_meta_collects_profile_and_optional_fields() -> None:
    ...
    assert "model_checkpoint_tree_sha256" not in payload
    assert payload["guard_metric_degradation_limit"] == 0.01
...
def test_missing_guard_pass_is_fail_closed_for_recovery_flag() -> None:
    assert build_flags_payload({"spectral": {"decision": "allow"}}) == {
        "guard_recovered": True,
        "rollback_reason": None,
    }
