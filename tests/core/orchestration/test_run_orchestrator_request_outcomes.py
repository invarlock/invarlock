...
def test_execute_run_request_emits_export_and_guard_metric_impact_failure(
    monkeypatch, tmp_path: Path
) -> None:
    class _Adapter:
        name = "stub"
    ...
    _install_common_monkeypatches(
        monkeypatch,
        adapter=_Adapter(),
        should_measure_metric_impact=(True, True, None),
    )
    services = _make_services(
        tmp_path,
        config,
    ...
    assert outcome.ok is False
    assert outcome.failure is not None
    assert outcome.failure.code == "guard_metric_impact_budget_exceeded"
...
def test_execute_run_request_covers_diagnostic_and_seed_fallback_branches(
    ...
    monkeypatch.setattr(
        "invarlock.core.orchestration.execute.np.random.get_state",
...
def test_execute_run_request_covers_retry_validation_and_timing_none(
    ...
    _install_common_monkeypatches(monkeypatch)
    monkeypatch.setattr(
        "invarlock.core.orchestration.execute._build_timing_summary_payload_impl",
        lambda **_kwargs: None,
    )
