...
import invarlock.core.scoring as metrics
...
from tests._evaluation_support import build_pack, rebind_pack, replay_pack
...
@pytest.mark.parametrize("kind", ["normalized_match", "token_f1"])
def test_policy_and_embedded_evidence_require_the_unicode_version(kind):
    ...
    with pytest.raises(EvaluationRecordsError, match="unicode_version"):
        validate(policy, "policy")
    with pytest.raises(EvidenceVerificationError, match="unicode_version"):
        replay_pack(
            evidence,
    ...
    evidence = rebind_pack(evidence, key, policy=policy)
    with pytest.raises(CapturedContractError, match="unicode_version"):
        load_payloads(evidence)
...
def test_signed_evidence_requires_matching_recipient_unicode_semantics(
    ...
    assert replay_pack(evidence, **expected)["decision"] == "pass"
    ...
    with pytest.raises(EvidenceVerificationError, match="request.*anchor|policy"):
        replay_pack(evidence, **{**expected, "policy": changed})
    monkeypatch.setattr(metrics.unicodedata, "unidata_version", OTHER_VERSION)
    with pytest.raises(
        EvidenceVerificationError, match="unsupported_scoring_environment"
    ):
        replay_pack(evidence, **expected)
