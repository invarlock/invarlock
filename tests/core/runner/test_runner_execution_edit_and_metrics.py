...
def test_independent_slice_summary_records_counts_and_degeneracy(tmp_path):
    import torch
    ...
    summary1 = metrics1["preview_final_slice_delta_summary"]
    assert summary1["preview_windows"] == 1
    assert summary1["final_windows"] == 1
    assert summary1["paired"] is False
    assert summary1["degenerate"] is True

    # Two disjoint slices with no within-slice variation.
    calibration2 = [
        {"input_ids": [1, 2, 3], "attention_mask": [1, 1, 1]},
        {"input_ids": [4, 5, 6], "attention_mask": [1, 1, 1]},
        {"input_ids": [7, 8, 9], "attention_mask": [1, 1, 1]},
    ...
    summary2 = metrics2["preview_final_slice_delta_summary"]
    assert summary2["preview_windows"] == 2
    assert summary2["final_windows"] == 2
    assert summary2["paired"] is False
    assert summary2["degenerate"] is True
