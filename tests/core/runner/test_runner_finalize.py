...
from invarlock.core.runner_runtime.finalize import handle_error
...
def test_finalize_rejects_nonpositive_ppl_preview():
    runner = CoreRunner()
    report = RunReport()
...
def test_finalize_rejects_missing_primary_metric() -> None:
    runner = CoreRunner()
    report = RunReport()
    guard_results = {"spectral": {"passed": True}}

    status = runner._finalize_phase(
        object(), object(), guard_results, {}, RunConfig(), report
    )
...
def test_finalize_rejects_explicit_non_evaluated_run() -> None:
    runner = CoreRunner()
    report = RunReport()
    metrics = {
        "eval_state": {
...
def test_finalize_rejects_non_evaluated_run_in_strict_assurance() -> None:
    runner = CoreRunner()
    report = RunReport()
    metrics = {
        "eval_state": {
    ...
    config = RunConfig(context={"assurance": {"mode": "strict"}})
...
def test_compute_real_metrics_collapsed_independent_slice_interval_is_not_invalid():
    ...
    assert summary["degenerate_reason"] == "constant_bootstrap_distribution"
    assert summary["paired"] is False
    assert metrics["primary_metric"]["invalid"] is False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"mode": "warn", "evaluated": False, "passed": None},
        {"mode": "WARN", "evaluated": True, "passed": True},
    ],
)
def test_finalize_phase_rejects_incomplete_or_coercible_tail(tail: object) -> None:
    runner = CoreRunner()
    report = RunReport()
    metrics = {
        "primary_metric": {"kind": "ppl_causal", "preview": 1.0, "final": 1.02},
        "primary_metric_tail": tail,
    }
