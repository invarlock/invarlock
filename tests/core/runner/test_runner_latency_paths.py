...
from invarlock.core.runner_runtime.eval_metrics import (
...
def test_measure_latency_reuses_tensor_auxiliary_storage(monkeypatch) -> None:
    calls: list[dict[str, torch.Tensor | None]] = []
    attention_mask = torch.tensor([[1, 1, 1]], dtype=torch.long)
    token_type_ids = torch.tensor([[0, 0, 1]], dtype=torch.long)
    real_tensor = torch.tensor
    ...
    class _CapturingModel:
        def __call__(
            ...
            return SimpleNamespace(loss=real_tensor(0.1))

    def reject_auxiliary_copy(value, *args, **kwargs):
        if value is attention_mask or value is token_type_ids:
            raise AssertionError("tensor-valued auxiliary input was copied")
        return real_tensor(value, *args, **kwargs)

    monkeypatch.setattr(torch, "tensor", reject_auxiliary_copy)

    latency = measure_latency(
        _CapturingModel(),
        [
            {
                "input_ids": real_tensor([1, 2, 3], dtype=torch.long),
                "attention_mask": attention_mask,
                "token_type_ids": token_type_ids,
            }
        ],
    ...
    assert calls[0]["token_type_ids"] is token_type_ids
