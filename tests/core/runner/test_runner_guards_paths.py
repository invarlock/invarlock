...
def test_runtime_policy_resolution_applies_release_profile_variance_override() -> None:
    runner = CoreRunner()
    report = RunReport(
        meta={"config": {"auto": {"tier": "balanced"}}},
        context={"profile": " Release "},
    )

    policies = runner._resolve_guard_policies(report)

    assert policies["variance"]["max_calib"] == 320
    assert policies["variance"]["calibration"] == {
        "windows": 8,
        "min_coverage": 6,
        "seed": 123,
    }
...
def test_guard_phase_sets_context_and_skips_timings_when_not_requested() -> None:
    ...
    guard = _ContextPrepareGuard(
        ...
        GuardValidationResult(
            ...
            extras={
                "final_metrics": {"layer": 1.0},
                "final_degeneracy": {
                    "layer": {"stable_rank": 1.0, "norm_collapse": 1.0}
                },
            },
        ),
    )
    ...
    assert results["spectral"]["final_degeneracy"] == {
        "layer": {"stable_rank": 1.0, "norm_collapse": 1.0}
    }
