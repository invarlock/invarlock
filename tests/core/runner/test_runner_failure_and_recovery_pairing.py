...
def test_independent_slice_summary_path(monkeypatch):
    runner = CoreRunner()
...
def test_independent_slice_singleton_interval_is_explicitly_degenerate():
    runner = CoreRunner()
    model = _toy_model_with_losses([1.0, 1.1])
    adapter = DummyAdapter()
    metrics, _ = runner._compute_real_metrics(
    ...
    summary = metrics.get("preview_final_slice_delta_summary", {})
    assert summary.get("degenerate") is True
    assert summary.get("degenerate_reason") == "constant_bootstrap_distribution"
...
def test_independent_slice_ci_receives_both_weight_vectors(monkeypatch):
    ...
    def fake_delta_ci(
        ...
        **kwargs,
        ...
        calls["final_weights"] = final_weights
        calls["preview_weights"] = preview_weights
        return (0.0, 0.0)

    monkeypatch.setattr(
        "invarlock.core.runner.compute_independent_delta_log_ci", fake_delta_ci
    )
    ...
    pm = metrics.get("primary_metric", {})
    assert pm.get("degraded") is False
    for key in ("final_weights", "preview_weights"):
        weights = calls.get(key)
