...
def test_eval_windows_are_disjoint_and_use_independent_slice_delta_summary():
    ...
    assert "paired_windows" not in metrics
    ...
    assert summary["ci_method"] == "independent_percentile_delta_log"


def test_paired_text_evaluation_emits_verified_pair_count_for_release_contract():
    calibration = _make_calibration(8)
    tokens = [batch["input_ids"].squeeze(0).tolist() for batch in calibration]
    model = DummyModel([3.0] * 8)
    runner = CoreRunner()
    config = RunConfig(
        context={
            "eval": {
            ...
            "dataset": {"seed": 42, "seq_len": 8, "stride": 8},
            "pairing_baseline": {
                "preview": {"window_ids": [0, 1, 2, 3], "input_ids": tokens[:4]},
                "final": {"window_ids": [4, 5, 6, 7], "input_ids": tokens[4:]},
            },
            "profile": "dev",
        }
    )
