...
import invarlock.core.runner_runtime.eval_metrics_multimodal as multimodal_metrics
from invarlock.adapters.hf_multimodal import HF_Multimodal_Adapter
from invarlock.core.run_policy import serialize_evaluation_windows
...
def test_compute_real_metrics_supports_vision_text_classification() -> None:
    ...
    assert eval_windows["preview"]["input_records"][0]["image_ref"] == "images/a.png"
    assert "image_path" not in eval_windows["preview"]["input_records"][0]
...
def test_compute_real_metrics_retries_multimodal_processor_without_truncation(
    ...
    assert eval_windows["final"]["records"][0]["correct"] is False
    assert eval_windows["preview"]["input_records"][0]["image_ref"] == (
        "images/demo.ppm"
    )
    assert "image_path" not in eval_windows["preview"]["input_records"][0]
...
def test_replay_window_serialization_removes_runtime_paths(tmp_path: Path) -> None:
    image = tmp_path / "dataset/images/demo.ppm"
    manifest = tmp_path / "dataset/manifest.jsonl"
    image_sha256 = "a" * 64
    record = multimodal_metrics._replay_input_record(
        {
            "image_path": str(image),
            "image_ref": "images/demo.ppm",
            "image_sha256": image_sha256,
            "source_file": str(manifest),
            "source_ref": "manifest.jsonl",
            "source_line": 1,
            "prompt": "what is shown?",
            "answers": ["cat"],
        },
    ...
    windows = serialize_evaluation_windows(
        {"preview": {"input_records": [record]}, "final": {}}
    )
    serialized = json.dumps(windows, sort_keys=True)

    assert str(tmp_path) not in serialized
    assert "image_path" not in record
    assert "source_file" not in record
    assert record["image_ref"] == "images/demo.ppm"
    assert record["source_ref"] == "manifest.jsonl"
    assert record["image_sha256"] == image_sha256


def test_replay_record_rejects_disguised_runtime_references(tmp_path: Path) -> None:
    image_sha256 = "b" * 64
    record = multimodal_metrics._replay_input_record(
        {
            "image_ref": str(tmp_path / "dataset/images/demo.ppm"),
            "image_sha256": image_sha256,
            "source_ref": "C:\\private\\manifest.jsonl",
        },
        example_id="ex-1",
    )

    assert record["image_ref"] == f"sha256:{image_sha256}"
    assert "source_ref" not in record
    assert (
        multimodal_metrics._portable_replay_reference(f"sha256:{image_sha256}")
        == f"sha256:{image_sha256}"
    )
    assert multimodal_metrics._portable_replay_reference(".") is None
