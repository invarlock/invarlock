...
from invarlock.runtime_providers import llama_cpp as llama_cpp
from invarlock.runtime_providers import llama_cpp_session as llama_cpp_session
...
def test_core_runtime_image_contains_the_llama_provider_without_addin_sources() -> None:
    repository = Path(__file__).resolve().parents[2]
    dockerfile = repository.joinpath("runtime/Dockerfile").read_text(encoding="utf-8")
    dockerignore = repository.joinpath(".dockerignore").read_text(encoding="utf-8")

    assert "COPY src /project/src" in dockerfile
    assert "llama_cpp" in dockerfile
    assert "addins" not in dockerfile
    assert "addins" not in dockerignore
    assert "--ignore-eos" not in dockerfile


def test_gguf_runtime_build_suppresses_only_backend_eog_console_marker() -> None:
    repository = Path(__file__).resolve().parents[2]
    dockerfile = repository.joinpath("runtime/Dockerfile.gguf").read_text(
        encoding="utf-8"
    )
    dockerignore = repository.joinpath(".dockerignore").read_text(encoding="utf-8")
    patch = repository.joinpath("runtime/llama-completion-user-output.patch").read_text(
        encoding="utf-8"
    )
