...
def test_native_capture_rehearsal():
    if os.environ.get("INVARLOCK_RUN_NATIVE_CAPTURE") != "1":
        pytest.skip(
            "set INVARLOCK_RUN_NATIVE_CAPTURE=1 with the explicit capture inputs"
        )
    output = Path(os.environ["INVARLOCK_NATIVE_OUTPUT"])
    command = [
        os.environ["INVARLOCK_NATIVE_PYTHON"],
        str(ROOT / "examples/captured-results/native_rehearsal.py"),
        "capture",
        "--evaluator",
