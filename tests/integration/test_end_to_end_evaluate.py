...
def test_tiny_attested_smoke_campaign_script_is_executable() -> None:
    ...
    assert '"${CLI[@]}" report html -i "$EVAL_REPORT" -o "$SMOKE_EXPORT_DIR/evaluation.html"' in contents
    assert 'proof_pack_unsigned=1; using explicit unattested-artifact override' in contents
