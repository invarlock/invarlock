...
import invarlock.guards.spectral_control as spectral_control_mod
import invarlock.guards.spectral_detection as spectral_detection_mod
import invarlock.guards.spectral_measurement as spectral_measurement_mod
...
guards_pkg.spectral_control = spectral_control_mod
guards_pkg.spectral_detection = spectral_detection_mod
guards_pkg.spectral_measurement = spectral_measurement_mod
...
def test_spectral_guard_validate_auto_prepare(monkeypatch):
    ...
    monkeypatch.setattr(
        spectral_measurement_mod, "capture_baseline_sigmas", fake_capture
    )
    monkeypatch.setattr(
        spectral_detection_mod, "classify_model_families", fake_classify
    )
    monkeypatch.setattr(
        spectral_detection_mod, "compute_family_stats", fake_family_stats
    )
    monkeypatch.setattr(spectral_measurement_mod, "scan_model_gains", lambda *_: {})
    monkeypatch.setattr(spectral_measurement_mod, "auto_sigma_target", lambda *_: 1.0)
    monkeypatch.setattr(
        spectral_control_mod,
        "apply_relative_spectral_cap",
...
def test_apply_relative_spectral_cap_handles_failure(monkeypatch):
    ...
    monkeypatch.setattr(spectral_measurement_mod, "compute_sigma_max", raise_error)
