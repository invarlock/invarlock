...
def test_normalize_family_caps_paths():
    ...
    # dict with numeric
    caps2 = normalize_family_caps({"ffn": {"kappa": 2.1}, "attn": 3.0})
    assert caps2["ffn"]["kappa"] == 2.1
    assert caps2["attn"]["kappa"] == 3.0

    # invalid → default when default=True
    caps3 = normalize_family_caps({"x": object()}, default=True)
    assert "ffn" in caps3

    # invalid → empty when default=False
    caps4 = normalize_family_caps({"x": object()}, default=False)
    assert caps4 == {}


def test_summarize_sigmas_and_z_scores():
    sigmas = {"a": 1.0, "b": 2.0, "c": 4.0}
    summary = summarize_sigmas(sigmas)
    ...
    zs = compute_z_scores(sigmas, family_stats, fam_map, sigmas, deadband=0.1)
    assert all(isinstance(v, float) for v in zs.values())

    # z-score fallback path (std=0)
    family_stats2 = {"ffn": {"mean": 2.0, "std": 0.0}}
    zs2 = compute_z_scores(sigmas, family_stats2, fam_map, sigmas, deadband=0.1)
    assert all(isinstance(v, float) for v in zs2.values())

    # summarize family z-scores
    fam_summary = summarize_family_z_scores(zs2, fam_map, {"ffn": {"kappa": 1.5}})
