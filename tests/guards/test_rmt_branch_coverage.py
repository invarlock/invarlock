...
def test_rmt_detect_report_smoke():
    model = TinyModel()
    summary, per_layer = rmt_detect_report(
...
def test_rmt_detect_verbose_flags_outliers():
    ...
    res = rmt_detect(model, threshold=1.0, verbose=True)
...
def test_analyze_weight_distribution_paths():
    ...
    empty = nn.Module()
    assert analyze_weight_distribution(empty) == {}
...
def test_rmt_detect_with_names_encoder_branch():
    ...
    em = EncoderModel()
    out = rmt_detect_with_names(em, threshold=10.0, verbose=False)
...
def test_rmt_detect_with_names_fallback_heuristic():
    ...
    m = OddModel()
    out = rmt_detect_with_names(m, threshold=10.0, verbose=False)
...
def test_clip_full_svd_nonfinite_short_circuits():
    ...
    out = clip_full_svd(W, clip_val=1.0, return_components=False)
    assert out is W
...
def test_small_helpers_deadband_and_growth_ratio():
    ...
    assert within_deadband(1.0, 1.0, 0.0) is True
...
def test_clip_full_svd_components_return():
    W = torch.randn(3, 2)
    U, S, Vt = clip_full_svd(W, clip_val=10.0, return_components=True)
