...
\paragraph{Why not existing standards?}
Experiment trackers (e.g., MLflow/W\&B) log metrics and artifacts but typically do not enforce a
closed schema with per-case verifier verdicts, prompt-set digesting rules, and code-execution
sandbox contracts.
SARIF targets static-analysis outputs, and OpenTelemetry targets service-level spans; neither
standardizes the determinism-oriented contract needed to compare two verifier runs of generative
models.
We fill a narrower gap: a strict, machine-validated object that binds auxiliary guard evidence to
first-class verifier traces under a shared contract.
...
(\texttt{schema\_version} const \path{verifier_carrying_artifact.v1}) intended to be the unit of
exchange for weight-edit evaluation evidence.
...
content-addressed file reference (SHA-256) and optional \texttt{invarlock verify} output (including
optional embedded \texttt{verify\_json}).
...
If contract fields match between two traces, the runs are \emph{intended} to be comparable; when
...
timeouts and resource limits), making execution constraints auditable rather than implicit. In v1
these include \texttt{timeout\_s}, \texttt{wall\_limit\_s}, \texttt{cpu\_limit}, and
\texttt{mem\_limit\_mb}, with \texttt{network\_enabled=false} schema-enforced.
...
(\texttt{s1\_demo.summary.jsonl}\footnote{\path{research/verifai2_2026/examples/s1_demo.summary.jsonl}})
illustrating joins and audit queries over verifier outcomes and guard evidence.
...
Two Phi-2 pruning variants are represented as BYOE subject checkpoints; this matters because the
...
\toprule
Model & Edit & Verifier & $\Delta$pass@1 & PM & Spec & RMT & Status \\
\midrule
DS-Coder-1.3B & INT8 RTN & HumanEval & +0.012 & \checkmark & \checkmark & \checkmark & capped \\
DS-Coder-1.3B & INT8 RTN & MBPP     & $-$0.006 & \checkmark & \checkmark & \checkmark & capped \\
Qwen-Coder-3B & INT8 RTN & HumanEval & $-$0.006 & \checkmark & \checkmark & \checkmark & capped \\
Phi-2 (BYOE)  & prune 40\% & HumanEval & +0.220 & $\times$ & \checkmark & \checkmark & stable \\
\bottomrule
\end{tabular}
\caption{Representative demo rows (joined view). ``capped'' means spectral family caps were applied
(\texttt{caps\_applied>0}); it is not itself a failure. Gate outcomes are recorded separately.
Abbreviations: DS-Coder = DeepSeek-Coder.}
\label{tab:demo}
\end{table}
