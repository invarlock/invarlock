...
This three-layer structure aligns with verifier-first evaluation: verifier-grounded outcomes are
the headline metric, while guard evidence is auxiliary and diagnostic.
...
target software artifacts, not model-evaluation evidence \citep{slsa2024spec}.
...
contracts.
SARIF standardizes static-analysis results but not generative-model evaluation.
OpenTelemetry traces capture service-level spans, not the determinism contract needed to compare
two verifier runs.
...
(e.g., HumanEval pass@1 \citep{chen2021humaneval}) to the concrete conditions under which it was
produced.
...
(i) structural conformance against Draft 2020-12 JSON Schema for both the wrapper artifact and each
verifier trace, and
