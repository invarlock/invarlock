...
#
# Uses the existing GPT-2 campaign so at least one slower lane runs on a model
...
echo "[info] $(ts) realistic smoke mode=$MODE work_root=$WORK_ROOT" | tee -a "$LOG_FILE"
echo "[info] $(ts) Log file: $LOG_FILE" | tee -a "$LOG_FILE"

set +e
INVARLOCK_SMOKE_MODE="$MODE" bash "$REPO_ROOT/scripts/run_gpt2_smoke_campaign.sh" "$WORK_ROOT" >>"$LOG_FILE" 2>&1
RC=$?
set -e
