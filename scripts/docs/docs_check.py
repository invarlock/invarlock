...
def check_links() -> None:
    ...
    code, out = run([sys.executable, "scripts/docs/check_docs_links.py"])
    print(out, end="")
    if code != 0:
        raise SystemExit(code)
    ...
    for md in Path("docs").rglob("*.md"):
        code, out = run(
            [sys.executable, "scripts/docs/check_internal_links.py", str(md)]
        )
...
def check_references() -> None:
    code, out = run([sys.executable, "scripts/docs/validate_doc_references.py"])
    print(out, end="")
    if code != 0:
        raise SystemExit(code)
...
def check_examples() -> None:
    code, out = run([sys.executable, "scripts/docs/validate_yaml_examples.py"])
    print(out, end="")
    if code != 0:
        raise SystemExit(code)

    code, out = run([sys.executable, "scripts/docs/validate_python_examples.py"])
    print(out, end="")
    if code != 0:
        raise SystemExit(code)

    # CLI examples extractor may be absent in minimal OSS; skip gracefully.
    cli_tester = Path("scripts/docs/test_cli_examples.py")
