...
test_run_qwen14_sentinels_helper_functions_cover_resolution_and_error_paths() {
    ...
    source ./scripts/proof_packs/run_qwen14_sentinels.sh
...
EOF
    ...
    run bash -x ./scripts/proof_packs/run_qwen14_sentinels.sh \
        --run-dir "${run_dir}" \
...
test_run_qwen14_sentinels_requires_inputs_and_rejects_bad_mode() {
    ...
    run bash -x ./scripts/proof_packs/run_qwen14_sentinels.sh --model-name qwen
    assert_rc "2" "${RUN_RC}" "missing run-dir returns usage error"

    local run_dir="${TEST_TMPDIR}/run"
    mkdir -p "${run_dir}"
    run bash -x ./scripts/proof_packs/run_qwen14_sentinels.sh --run-dir "${run_dir}" --model-name qwen --mode nope
