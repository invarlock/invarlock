...
test_setup_remote_install_invarlock_stack_uses_pinned_requirement_repairs() {
    ...
    touch \
        "${REPO_DIR}/requirements/proof-packs/huggingface_hub.txt" \
        "${REPO_DIR}/requirements/proof-packs/accelerate.txt" \
        "${REPO_DIR}/requirements/proof-packs/pyyaml.txt" \
        "${REPO_DIR}/requirements/proof-packs/protobuf.txt" \
        "${REPO_DIR}/requirements/proof-packs/sentencepiece.txt"
    ...
    assert_match "requirements/proof-packs/huggingface_hub.txt" "${log_text}" "huggingface_hub pinned repair"
    assert_match "requirements/proof-packs/accelerate.txt" "${log_text}" "accelerate pinned repair"
    assert_match "requirements/proof-packs/pyyaml.txt" "${log_text}" "pyyaml pinned repair"
    assert_match "requirements/proof-packs/protobuf.txt" "${log_text}" "protobuf pinned repair"
    assert_match "requirements/proof-packs/sentencepiece.txt" "${log_text}" "sentencepiece pinned repair"
}
...
if [[ "${cmd}" == "clone" ]]; then
    ...
    mkdir -p \
        "${dest}/.git" \
        "${dest}/requirements/proof-packs" \
        "${dest}/scripts/proof_packs" \
        "${dest}/scripts/proof_packs/python"
    touch "${dest}/scripts/proof_packs/run_suite.sh" \
        "${dest}/scripts/proof_packs/run_pack.sh" \
        "${dest}/scripts/proof_packs/verify_pack.sh" \
        "${dest}/scripts/proof_packs/python/remote_setup_smoke.py" \
        "${dest}/requirements/proof-packs/huggingface_hub.txt" \
        "${dest}/requirements/proof-packs/accelerate.txt" \
        "${dest}/requirements/proof-packs/pyyaml.txt" \
        "${dest}/requirements/proof-packs/protobuf.txt" \
        "${dest}/requirements/proof-packs/sentencepiece.txt"
    exit 0
fi
exit 0
EOF
...
if [[ "${1:-}" == "${REPO_DIR}/scripts/proof_packs/python/remote_setup_smoke.py" ]]; then
...
EOF
    ...
    assert_match "requirements/proof-packs/huggingface_hub.txt" "${pip_log}" "huggingface_hub pin installed"
    assert_match "requirements/proof-packs/accelerate.txt" "${pip_log}" "accelerate pin installed"
    assert_match "requirements/proof-packs/pyyaml.txt" "${pip_log}" "pyyaml pin installed"
    assert_match "requirements/proof-packs/protobuf.txt" "${pip_log}" "protobuf pin installed"
    assert_match "requirements/proof-packs/sentencepiece.txt" "${pip_log}" "sentencepiece pin installed"
}
