...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-check a proof-pack remote environment."
    )
    parser.add_argument(
        "--module",
        dest="modules",
        action="append",
        default=[],
        help="Extra module import to require. Defaults cover the proof-pack stack.",
    )
    parser.add_argument(
        "--cli",
        default="invarlock",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if missing:
        print(
            "ERROR: Missing proof-pack remote modules: " + ", ".join(missing),
            file=sys.stderr,
        )
        return 1
