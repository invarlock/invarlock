...
class SourceRepoMetadataError(RuntimeError):
    """Raised when proof-pack source provenance cannot be collected safely."""
    ...
    *args: str,
    ...
    try:
        ...
        if required:
            raise SourceRepoMetadataError(
                "git is required to collect proof-pack source provenance."
            ) from exc
        return ""
    if proc.returncode != 0:
        ...
        raise SourceRepoMetadataError(
            "git "
            + " ".join(args)
            + " failed while collecting proof-pack source provenance: "
            + detail
        )
