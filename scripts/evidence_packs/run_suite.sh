#!/usr/bin/env bash
# run_suite.sh - CLI entrypoint for evidence pack suites.
...
pack_usage() {
    cat <<'EOF'
InvarLock Evidence Pack Suite
Usage: scripts/evidence_packs/run_suite.sh [options]
...
Options:
  ...
  --out DIR            Output directory (default: ./evidence_pack_runs/<suite>_<timestamp>)
...
pack_require_entrypoint_security_opt_ins() {
    ...
    echo "ERROR: evidence-pack bulk runs require INVARLOCK_ALLOW_REMOTE_CODE=1." >&2
