...
REMOTE_SMOKE_CLI_CHECKS = (
    ("advanced", "evidence-pack", "--help"),
    ("report", "validate", "--help"),
)
REMOTE_SMOKE_REPO_ENTRYPOINTS = (
...
def required_modules(extra_modules: list[str]) -> tuple[str, ...]:
    if not extra_modules:
        return DEFAULT_REMOTE_SMOKE_MODULES
    return tuple(dict.fromkeys([*DEFAULT_REMOTE_SMOKE_MODULES, *extra_modules]))
...
def check_cli(cli_name: str) -> str | None:
    ...
    for args in REMOTE_SMOKE_CLI_CHECKS:
        proc = subprocess.run(
            [cli_name, *args],
            check=False,
            capture_output=True,
...
def check_repo_root(repo_root: str) -> str | None:
    ...
    for relpath in REMOTE_SMOKE_REPO_ENTRYPOINTS:
...
def remote_setup_smoke(args: argparse.Namespace) -> int:
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    remote_parser = subparsers.add_parser(
        "remote-setup-smoke", help="Smoke-check an evidence-pack remote environment."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "env-report":
        return env_report()
    if args.cmd == "remote-setup-smoke":
        return remote_setup_smoke(args)
