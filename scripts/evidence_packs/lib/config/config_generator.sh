#!/usr/bin/env bash
# config_generator.sh - InvarLock config generation + evaluate helpers for evidence packs.
...
source "${_PACK_CONFIG_GENERATOR_DIR}/../core/runtime.sh"

# ============ INVARLOCK CONFIG FOR EVIDENCE PACKS ============
generate_invarlock_config() {
    ...
    _emit_config_yaml() {
        cat << YAML_EOF
# Auto-generated InvarLock config for evidence packs
# Platform: evidence pack runner
...
run_single_calibration() {
    ...
    if [[ -n "${report_file}" ]]; then
        ...
        _cmd_python "${_PACK_CONFIG_GENERATOR_PY_DIR}/task_tools.py" evaluation-report \
            --report "${report_file}" \
...
# ============ evaluate WITH EVIDENCE PACK SETTINGS ============
