...
execute_task() {
    ...
    (
        ...
        case "${task_type}" in
            ...
            evaluate_EDIT_GROUP)
                local entries
                entries=$(echo "${params}" | jq -c '.entries // []')
                task_evaluate_edit_group "${model_name}" "${gpu_id}" "${entries}" "${output_dir}" "${task_log}" || exit_code=$?
                ;;
            CLEANUP_EDIT)
                local edit_spec=$(echo "${params}" | jq -r '.edit_spec // ""')
                local version=$(echo "${params}" | jq -r '.version // "clean"')
...
# ============ TASK: evaluate_EDIT_GROUP ============

# Run multiple edit evaluations in one Python process. This keeps each subject
# checkpoint isolated but avoids restarting the CLI/Python interpreter for every
# scenario in the group.
# Usage: task_evaluate_edit_group <model_name> <gpu_id> <entries_json> <output_dir> <log_file>
task_evaluate_edit_group() {
    ...
    if ! command -v jq >/dev/null 2>&1; then
        echo "  WARNING: jq unavailable; falling back to per-edit evaluation tasks" >> "${log_file}"
        return 1
    fi
    ...
    if [[ -z "${baseline_report_file}" || ! -f "${baseline_report_file}" ]]; then
        echo "  WARNING: Baseline report unavailable for grouped evaluation" >> "${log_file}"
        return 1
    fi

    local group_root="${model_output_dir}/evaluation_groups/${TASK_ID:-manual}"
    mkdir -p "${group_root}" "${output_dir}/results/analysis"
    local entry_lines="${group_root}/entries.jsonl"
    : > "${entry_lines}"
    ...
    while IFS= read -r entry; do
        ...
        local edit_spec version run
        edit_spec=$(printf '%s' "${entry}" | jq -r '.edit_spec // empty')
        version=$(printf '%s' "${entry}" | jq -r '.version // "clean"')
        run=$(printf '%s' "${entry}" | jq -r '.run // 1')
        if [[ -z "${edit_spec}" ]]; then
            echo "ERROR: grouped evaluate entry missing edit_spec" >> "${log_file}"
            return 1
        fi
        ...
        local edit_path="${model_output_dir}/models/${edit_dir_name}"
        local cert_dir="${model_output_dir}/reports/${edit_dir_name}/run_${run}"
        local cert_file="${cert_dir}/evaluation.report.json"

        if [[ -f "${cert_file}" ]]; then
            echo "  Evaluation report for ${edit_dir_name} run ${run} already exists, skipping" >> "${log_file}"
            continue
        fi
        if [[ ! -d "${edit_path}" ]]; then
            echo "ERROR: Edit model not found: ${edit_path}" >> "${log_file}"
        ...
        staged_baseline_report="$(_stage_baseline_report_for_eval "${baseline_report_file}" "${cert_dir}" "${log_file}")" || {
            echo "ERROR: Failed to stage baseline report for evaluate runtime: ${baseline_report_file}" >> "${log_file}"
            return 1
        }
        ...
        local config_root="${cert_dir}/config_root"
        mkdir -p "${config_root}/runtime/profiles"
        cat > "${config_root}/runtime/profiles/ci.yaml" << YAML
model:
  device_map: "auto"
...
PRESET_YAML
        ...
        local allow_remote_code="false"
        if pack_remote_code_allowed; then
            allow_remote_code="true"
        fi
        local defer_rendering="false"
        if _pack_defer_report_rendering_enabled; then
        ...
        jq -cn \
            ...
            --arg version "${version}" \
            --argjson run "${run}" \
            --arg baseline "${abs_baseline_path}" \
            --arg baseline_report "${staged_baseline_report}" \
            --arg subject "${abs_edit_path}" \
            --arg edit_label "${edit_label}" \
            --arg profile "${profile_flag}" \
            --arg tier "${tier}" \
            --arg out "${cert_dir}" \
            --arg report_out "${cert_dir}" \
    ...
    if [[ ${prepared_count} -eq 0 ]]; then
        echo "  No grouped edit evaluations needed" >> "${log_file}"
        return 0
    fi

    local entries_file="${group_root}/entries.json"
    jq -s '{schema:"invarlock/evidence-pack-evaluate-group-v1", entries:.}' "${entry_lines}" > "${entries_file}"
