...
test_result_compiler_generate_verdict_passes_manifest_when_present() {
    ...
    printf '%s\n' '{"schema":"evidence_pack_scenarios_v1","schema_version":1,"scenarios":[]}' > "${OUTPUT_DIR}/state/scenarios.json"

    log_section() { :; }
    log() { printf '%s\n' "$*" >> "${TEST_TMPDIR}/result_compiler.log"; }
    _pack_result_compiler_root() { echo "${TEST_ROOT}/scripts/evidence_packs"; }
    python3() {
