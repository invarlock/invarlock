...
def _iter_absolute_path_strings(payload: Any) -> set[Path]:
    ...
    if isinstance(payload, str):
        ...
        if candidate.is_absolute():
            paths.add(_absolute_path_no_resolve(candidate))
        return paths
    if isinstance(payload, dict):
        for value in payload.values():
            paths.update(_iter_absolute_path_strings(value))
        return paths
    if isinstance(payload, (list, tuple, set)):
        for item in payload:
            paths.update(_iter_absolute_path_strings(item))
    return paths


def _absolute_path_no_resolve(path: str | Path) -> Path:
...
def _create_loader(
    ...
    *,
    ...
    def _construct_include(loader: yaml.SafeLoader, node: yaml.Node):
        ...
        if not _allow_config_include_outside():
            try:
        ...
        dependency_paths = getattr(loader, "_dependency_paths", None)
...
def _load_raw_config_payload(
    ...
    *,
    ...
    resolved = _absolute_path_no_resolve(p)
    if isinstance(dependency_paths, set):
...
def inspect_config_dependencies(path: str | Path) -> ConfigDependencyScan:
    ...
    referenced_paths = _iter_absolute_path_strings(raw)
    return ConfigDependencyScan(
        ...
        referenced_paths=tuple(
            sorted(
                {_absolute_path_no_resolve(item) for item in referenced_paths},
                key=str,
            )
        ),
    )
