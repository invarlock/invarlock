...
@report_app.command(
    name="export",
    help=(
        "Export evaluation evidence for MLflow tags, model cards, or release review."
    ),
)
def report_export(
    ...
    verify_result: str | None = typer.Option(
        ...
        "--verify-result",
        help=(
            "Path to `invarlock verify --json` output. When supplied, export "
            "status and verifier fields come from the verifier result."
        ),
    ),
    force: bool = typer.Option(
