...
from . import report_validation_overhead as _overhead

GetTierPoliciesFn = Callable[[], dict[str, Any]]
_guard_overhead_has_error_diagnostic = _overhead.guard_overhead_has_error_diagnostic
_resolve_guard_overhead_pass = _overhead.resolve_guard_overhead_pass
    ...
    if isinstance(primary_metric, dict):
        ...
        if pm_kind == "accuracy":
            ...
            if pm_preview is not None and pm_final is not None:
                ...
                preview_final_drift_acceptable = abs(pm_final - pm_preview) <= max(
                    0.0, accuracy_delta_limit
                )
    if tiny_relax:
