...
def _is_non_assurance_report(evaluation_report: dict[str, Any]) -> bool:
    """Identify reports that do not even declare strict assurance.

    This is deliberately narrower than a trust decision.  A standalone
    renderer cannot authenticate report-authored verdict or provenance fields,
    so every rendered report receives the unconditional report-local banner
    below regardless of this declaration.
    """
...
def _append_primary_metric_section(
    ...
    if kind == "accuracy":
        if isinstance(delta_pp, int | float):
            lines.append(f"| Δ vs Baseline | {float(delta_pp):+.2f} pp |")
        else:
...
def _append_dataset_details(lines: list[str], dataset: dict[str, Any]) -> None:
    ...
    if isinstance(tokenizer, dict) and (tokenizer.get("name") or tokenizer.get("hash")):
...
def _append_provenance_details(
    lines: list[str],
...
def _append_dataset_and_provenance_section(
    ...
    lines.extend(("## Dataset and Provenance", ""))
    if has_dataset:
        _append_dataset_details(lines, dataset)
    if has_provenance:
        _append_provenance_details(lines, evaluation_report, provenance_info)
...
def _append_report_warning_banners(
    ...
    warnings: list[str] = [
        "REPORT-LOCAL / UNVERIFIED RENDER — this renderer does not independently "
        "verify report bytes, provenance, policy inputs, or declared assurance fields"
    ]
...
def _append_safety_dashboard_section(
    ...
    lines.append("|-------|--------|---------------|")
    lines.append(
        f"| Report-local gates | {summary.overall_status} | Canonical gate outcomes |"
    )
...
def _append_executive_summary_section(
    ...
    lines.append("")
    summary = build_safety_dashboard_summary(evaluation_report)
    lines.append(f"**Report-local Gate Status:** {summary.overall_status}")
