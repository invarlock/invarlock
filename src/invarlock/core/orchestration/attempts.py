...
from invarlock.core.orchestration import attempt_results as _attempt_results
...
_enforce_guard_metric_impact_budget = (
    _attempt_results._enforce_guard_metric_impact_budget
)
...
def _build_skipped_guard_metric_impact_payload(
    *,
    ...
    skip_reason = (
        ...
        else "context.eval.skip_guard_metric_impact_check"
    )
    return {
        ...
        "mode": "skipped",
        "source": skip_guard_metric_impact_source
        or "config:context.run.skip_guard_metric_impact_check",
        "diagnostics": [
            {
                "kind": "guard_metric_impact_info",
                "severity": "info",
                "message": "Guard metric impact check skipped via config policy",
                "details": {},
            }
        ],
        "checks": {},
...
def _execute_attempt_core(
    *,
    ...
    guard_metric_impact_payload: dict[str, Any] | None = None
    try:
        if skip_guard_metric_impact and profile_normalized in {"ci", "release"}:
            guard_metric_impact_payload = _build_skipped_guard_metric_impact_payload(
        ...
        elif measure_guard_metric_impact:
            guard_metric_impact_payload = run_bare_control_fn(
                adapter=adapter,
                edit_op=edit_op,
                cfg=cfg,
    ...
    return _AttemptExecutionState(
        attempt=attempt,
        edit_config=edit_config,
        guard_metric_impact_payload=guard_metric_impact_payload,
