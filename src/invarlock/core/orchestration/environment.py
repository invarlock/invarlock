...
from invarlock.core.run_policy import (
    should_measure_metric_impact as _should_measure_metric_impact_default,
)
...
def _extract_tokenizer_load_kwargs_from_cfg(cfg: Any) -> dict[str, Any]:
    ...
    raw_model_identity: Any = None
    if isinstance(model_section, dict):
        trust_remote_code = model_section.get("trust_remote_code")
        raw_model_identity = model_section.get("model_identity")
        ...
        try:
            raw_model_identity = getattr(model_section, "model_identity", None)
        except (AttributeError, TypeError, ValueError):
            raw_model_identity = None

    kwargs: dict[str, Any] = {}
    if isinstance(trust_remote_code, bool):
        kwargs["trust_remote_code"] = trust_remote_code
    model_identity = validated_model_identity(raw_model_identity)
    if model_identity is not None and model_identity["kind"] == "remote_revision":
        kwargs["revision"] = model_identity["revision"]
    return kwargs
...
def _prepare_run_environment(
    *,
    ...
    if resolved_config_out is not None:
        prepare_kwargs["resolved_config_out"] = resolved_config_out
    ...
    should_measure_metric_impact_fn = getattr(
        _execute_helpers_module,
        "_should_measure_metric_impact_impl",
        _should_measure_metric_impact_default,
    )
    (
        measure_guard_metric_impact,
        skip_guard_metric_impact,
        skip_guard_metric_impact_source,
    ) = should_measure_metric_impact_fn(profile_normalized, cfg)
