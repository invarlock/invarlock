"""Post-attempt result processing for config-driven run orchestration."""
...
def _emit_primary_metric_summary_from_report(
    *,
    ...
    try:
        ...
        ratio_vs_base = (
            pm_obj.get("ratio_vs_baseline") if pm_kind.startswith("ppl") else None
        )
...
def _enforce_guard_metric_impact_budget(
    *,
    report: dict[str, Any],
    measure_guard_metric_impact: bool,
    guard_metric_degradation_limit: float,
    emit_guard_metric_impact_summary: Any,
    halt: Any,
) -> None:
    guard_metric_impact_info = report.get("guard_metric_impact")
    if not measure_guard_metric_impact:
        if guard_metric_impact_info:
            emit_guard_metric_impact_summary(
                guard_metric_impact_info,
                default_limit=guard_metric_degradation_limit,
            )
        return
    if not isinstance(guard_metric_impact_info, dict) or not guard_metric_impact_info:
        halt("guard_metric_impact_unavailable", reason="missing evidence")
        return
    if guard_metric_impact_info.get("passed") is True:
        contract_errors = guard_metric_impact_payload_errors(
            guard_metric_impact_info,
        ...
        if contract_errors:
            halt(
                "guard_metric_impact_unavailable",
                reason="canonical contract invalid",
            )
            return
    emit_guard_metric_impact_summary(
        guard_metric_impact_info,
    ...
    degradation_limit_raw = guard_metric_impact_info.get(
        "degradation_limit", guard_metric_degradation_limit
    )
    if isinstance(degradation_limit_raw, bool):
        halt("guard_metric_impact_unavailable", reason="invalid degradation limit")
        return
    try:
        degradation_limit = float(degradation_limit_raw)
    except (TypeError, ValueError):
        halt("guard_metric_impact_unavailable", reason="invalid degradation limit")
        return
    if not math.isfinite(degradation_limit) or degradation_limit < 0.0:
        halt("guard_metric_impact_unavailable", reason="invalid degradation limit")
        return

    degradation_raw = guard_metric_impact_info.get("degradation")
    if degradation_raw is None or isinstance(degradation_raw, bool):
        halt("guard_metric_impact_unavailable", reason="invalid degradation")
        return
    try:
    ...
    if guard_metric_impact_info.get("evaluated") is not True:
        halt("guard_metric_impact_unavailable", reason="measurement not evaluated")
        return
    if guard_metric_impact_info.get("passed") is not True:
        exceeded_context: dict[str, float | str] = {
            "degradation_limit": degradation_limit
        }
        degradation_basis = guard_metric_impact_info.get("degradation_basis")
        if isinstance(degradation_basis, str):
            exceeded_context["degradation_basis"] = degradation_basis
        halt(
    ...
    if degradation > degradation_limit:
        failure_context: dict[str, float | str] = {
            "degradation_limit": degradation_limit
        }
        degradation_basis = guard_metric_impact_info.get("degradation_basis")
        if isinstance(degradation_basis, str):
            failure_context["degradation_basis"] = degradation_basis
        halt(
            "guard_metric_impact_budget_exceeded",
            **failure_context,
        )
...
def _process_attempt_result(
    *,
    ...
    assembly_result = assemble_run_report_fn(
        ...
        timings=timings,
        guard_metric_impact_payload=attempt_state.guard_metric_impact_payload,
        baseline=baseline,
    ...
    _enforce_guard_metric_impact_budget(
        report=report,
        measure_guard_metric_impact=measure_guard_metric_impact,
        guard_metric_degradation_limit=guard_metric_degradation_limit,
