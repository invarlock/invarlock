...
def finalize_phase(
    ...
    raw_metric_payload_invalid = bool(
    ...
    metric_payload_invalid = raw_metric_payload_invalid
        ...
        if has_preview and has_final:
            if isinstance(pm_fin, int | float) and isinstance(pm_prev, int | float):
                ...
                if math.isfinite(pm_prev_val) and math.isfinite(pm_fin_val):
                    if pm_prev_val > 0.0 and pm_fin_val > 0.0:
    ...
    tail_failed = False
    if isinstance(metrics, dict) and "primary_metric_tail" in metrics:
        try:
            tail_outcome = require_primary_metric_tail(metrics["primary_metric_tail"])
        except PrimaryMetricTailContractError:
            tail_payload_invalid = True
        else:
            tail_failed = not tail_outcome.acceptable
