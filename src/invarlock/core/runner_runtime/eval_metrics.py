...
from .eval_latency import (
    _raise_latency_error,
    measure_latency,
    samples_to_dataloader,
)
from .eval_metrics_multimodal import (
...
from .eval_metrics_stats import (
...
from .eval_windows import compute_slice_summary, resolve_limit, slice_calibration
from .pairing import (
...
def _resolve_eval_runtime_context(
    ...
    *,
    ...
    # Preview and final are disjoint slices, so their difference always uses
    # independent-arm percentile resampling. Paired BCa remains reserved for
    # identical-ID baseline/subject windows during report construction.
...
def _build_real_metrics_payload(
    ...
    metrics = {
        ...
        "preview_final_slice_delta_summary": {
            "mean": float(slices.delta_mean_log),
            "ci": [
                float(bootstrap.delta_log_ci[0]),
                float(bootstrap.delta_log_ci[1]),
            ],
            "basis": "independent_disjoint_slices",
            "paired": False,
            "ci_method": bootstrap.delta_ci_method,
            "ci_reason": bootstrap.delta_ci_reason,
    ...
    if runtime.pairing_context:
        metrics["paired_windows"] = min(
            int(pairing.preview_pair_stats.get("matched", 0)),
            int(pairing.final_pair_stats.get("matched", 0)),
        )
...
def compute_real_metrics(
    ...
    *,
    compute_independent_delta_log_ci_fn: Any = compute_independent_delta_log_ci,
    ...
    bootstrap = _compute_bootstrap_delta_stats(
        ...
        runtime,
        slices,
        compute_independent_delta_log_ci_fn=compute_independent_delta_log_ci_fn,
