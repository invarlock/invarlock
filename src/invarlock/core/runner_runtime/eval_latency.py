...
def measure_latency(model: Any, sample_data: Any, device: Any) -> float:
    ...
    with torch.no_grad():
        ...
        if isinstance(sample, dict) and "attention_mask" in sample:
            try:
                attn_tensor = sample["attention_mask"]
                if not isinstance(attn_tensor, torch.Tensor):
                    attn_tensor = torch.tensor(attn_tensor)
            except _LATENCY_ERRORS as error:
                _raise_latency_error(
        ...
        if isinstance(sample, dict) and "token_type_ids" in sample:
            try:
                token_type_t = sample["token_type_ids"]
                if not isinstance(token_type_t, torch.Tensor):
                    token_type_t = torch.tensor(token_type_t)
