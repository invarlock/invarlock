"""Small, versioned deterministic scorers shared by evaluation paths."""
...
class UnsupportedScoringEnvironment(MetricError):
    """Valid scoring intent cannot execute on this host."""

    reason = "unsupported_scoring_environment"


class InvalidScoringReference(MetricError):
    """A deterministic scorer cannot use the submitted reference."""


def require_scoring_environment(
    kind: str, configuration: dict[str, Any], *, unicode_version: str | None = None
) -> None:
    version = (
        unicodedata.unidata_version if unicode_version is None else unicode_version
    )
    if (
        kind in ("normalized_match", "token_f1")
        and configuration["unicode_version"] != version
    ):
        raise UnsupportedScoringEnvironment(
            f"{kind} requires Unicode {configuration['unicode_version']!r}; "
            f"this runtime provides {version}. "
            "Use a runtime with the policy's Unicode version."
        )


def validate_configuration(
    kind: str, configuration: dict[str, Any], *, portable: bool = False
) -> None:
    allowed = {
        "exact_match": set(),
    ...
    if kind in ("normalized_match", "token_f1"):
        ...
        if portable:
            version = configuration["unicode_version"]
            if (
                not isinstance(version, str)
                or re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+", version) is None
            ):
                raise MetricError("invalid portable Unicode scoring configuration")
        else:
            require_scoring_environment(kind, configuration)
...
def score(
    ...
    if kind == "numeric_tolerance":
        try:
            ...
            raise InvalidScoringReference("numeric reference must be finite") from exc
        try:
