...
class TensorRTLLMProvider:
    ...
    def validate_config(self, spec: ModelRuntimeSpec) -> None:
        ...
        if spec.adapter_name is not None:
            raise ValueError("tensorrt_llm does not accept an in-process model adapter")
    ...
    def capabilities(self) -> RuntimeProviderCapabilities:
        return RuntimeProviderCapabilities(
            ...
            required_extra=None,
            required_image=None,
            platform_constraints=("linux", "nvidia_gpu", "tensorrt_llm_1_2_1"),
    ...
    def open(
        ...
        if not isinstance(context.native_model, TensorRTLLMRuntimeBindings):
            raise ValueError(
        ...
        if context.model_adapter is not None or context.scorer is not None:
            raise ValueError(
                "tensorrt_llm does not accept in-process adapter or scorer bindings"
            )
        ...
        observed_identity = read_tensorrt_llm_artifact_identity(
            context.native_model.engine_bundle_path,
            target_compute_capability=identity.target_compute_capability,
            tokenizer_metadata_sha256=identity.tokenizer_metadata_sha256,
        )
