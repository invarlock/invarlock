"""Closed TensorRT-LLM 1.2.1 connector used by the runtime image.
    ...
    try:
        with _silence_backend_output():
            ...
            if not isinstance(text, str):
                raise TensorRTLLMRunnerError("generation output is not text")
            if len(text.encode("utf-8", errors="strict")) > _MAX_OUTPUT_BYTES:
