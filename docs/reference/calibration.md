...
DeepSeek-R1-Distill-Qwen 14B, and Phi-4 text-only profiles. The repo also
...
assurance basis until supporting artifacts are attached. Multimodal calibration
configs that use `vision_text` expect the referenced local manifest to be
materialized before the sweep runs.
