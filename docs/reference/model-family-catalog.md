...
| Gemma 4 12B any-to-any LM | `community_experimental` | `google/gemma-4-12B-it` | Pilot `vision_text` preset, sweep lane, and Gemma 4 unified loader tests are included. This lane uses `hf_multimodal` with `invarlock[multimodal]`; promotion still requires public image-text evidence rather than the local smoke manifest. |
