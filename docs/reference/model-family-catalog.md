...
| Gemma 4 12B any-to-any LM | `community_experimental` | `google/gemma-4-12B-it` | Prepared image-text lane with archived preservation artifacts. The archived VQAv2 no-op run passed preservation policy but produced 0.035 final exact-answer accuracy, below the published image-text basis floor; audio evaluation remains deferred. |
| Qwen3.5 4B image-text LM | `community_experimental` | `Qwen/Qwen3.5-4B` | Prepared image-text candidate lane with explicit `hf_multimodal` preset, public VQAv2 materialization, and null-sweep calibration config. A 2026-06-14 H100 no-op run passed mechanically but produced 0/400 baseline and subject VQA accuracy. After the multimodal chat-template argument-forwarding fix and a strict answer-only prompt rerun, a 16/16 H100 sanity still produced 0/32 exact-answer VQA accuracy with instruction-analysis outputs, so public-basis promotion is blocked until the preset/model choice yields a meaningful primary metric. |
...
| Gemma 4 26B-A4B MoE image-text LM | `community_experimental` | `google/gemma-4-26B-A4B-it` | Prepared image-text MoE lane with archived preservation artifacts. The archived VQAv2 no-op run passed preservation policy but produced 0.045 final exact-answer accuracy, below the published image-text basis floor; it is not audio or MoE routing-quality evidence. |
...
| Qwen family aliases (Qwen1.5/Qwen2.5/Qwen3 naming) | `profile_first_class` | `Qwen/Qwen2.5-14B`, `Qwen/Qwen3.5-9B`, `Qwen/Qwen3.5-4B` | Shared qwen-family heuristics cover aliases beyond the declared text-only Qwen2, Qwen2.5 14B, Qwen3, and Qwen3.5 9B public lanes. Qwen3.5 2B has published image-text evidence through `hf_multimodal` and pinned public VQAv2 materialization; Qwen3.5 4B remains prepared but blocked after strict-prompt H100 sanity reproduced zero exact-answer VQA accuracy. |
...
| Gemma family | `profile_first_class` | `google/gemma-4-E2B-it`, `google/gemma-4-12B-it`, `google/gemma-4-26B-A4B-it` | Gemma-family selectors and loaders remain first-class for compatible local or user-supplied checkpoints. Repo-declared Gemma support is limited to Gemma 4 lanes with public evidence; Gemma 3 and Gemma 3n hosted checkpoints are omitted from the included support/promotion inventory because their Hugging Face pages are gated under Gemma terms. |
...
| GLM | `auto_or_loader_only` | `local-glm-compatible-checkpoint` | Visible through adapter-auto heuristics only. The hosted GLM-4 9B Chat checkpoint is intentionally not named because its public license is custom/restricted rather than Apache-2.0 or MIT. |
