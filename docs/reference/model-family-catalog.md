...
| Gemma 4 12B any-to-any LM | `community_experimental` | `google/gemma-4-12B-it` | Pilot `vision_text` preset, pinned public VQAv2 materialization lane, and Gemma 4 unified loader tests are included. This lane uses `hf_multimodal` with `invarlock[multimodal]`; promotion still requires a clean public release evidence run and evidence pack. |
