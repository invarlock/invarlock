...
| Qwen family aliases (Qwen1.5/Qwen2.5/Qwen3 naming) | `profile_first_class` | `Qwen/Qwen2.5-14B`, `Qwen/Qwen3.5-9B` | Shared qwen-family heuristics cover aliases beyond the declared Qwen2, Qwen2.5 7B, Qwen2.5 14B, Qwen3, and Qwen3.5 lanes, including larger usage-only Qwen2.5 checkpoints. |
...
| Family | State | Representative models | Notes |
| --- | --- | --- | --- |
| Qwen2.5 32B | `usage_only` | `Qwen/Qwen2.5-32B` | Used in evidence-pack suites and validation defaults outside the declared Qwen2.5 7B and Qwen2.5 14B support lanes. |
...
Only Qwen2.5 7B clears the full bar in this wave. The remaining `<=14B` text
...
| --- | --- | --- | --- | --- |
| Qwen2.5 7B causal LM | `Qwen/Qwen2.5-7B` | `promote_now` | `usage_only` | Cleared the six-part bar in this wave. |
...
| DistilBERT base uncased MLM | `distilbert-base-uncased` | `blocked_missing_artifacts` | `implemented_coverage` | Loader and adapter tests exist, but the broader MLM lane artifacts are absent. |
