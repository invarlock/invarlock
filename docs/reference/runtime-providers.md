...
Face vision-text are built-in providers with optional backend dependencies.
...
requires text task inputs and answers, so a vision-text capability alone
...
| `llama_cpp` | `invarlock` | `gguf` | `exact_match` | `container`, `local_process` | `cpu`; strict evidence uses an authenticated side worker |
...
vision-text likewise expose only exact-match collection; authenticated text
outputs can also support a compatible scorer extension or native judge profile. A provider may not emit a metric it did not advertise, and
...
| Artifact format | Identity fields beyond `format_version` and `artifact_format` |
| --- | --- |
...
| Output text and output digest | Required for `exact_match`, native judge and scorer-extension collection | Forbidden |
...
provider runtime selected for the transaction.

The core provider and three named built-in runtime providers are recognized without
...
Install the core wheel into the qualification host interpreter and check
...
Check the built-in TensorRT-LLM provider the same way:
...
Install and check the vision-text extra. Its Pillow dependency supports host
preflight; the inference stack remains in the runtime image:

```bash
"$PYTHON" -m pip install 'invarlock[vision-text]'
...
Artifact inspection for either native provider executes backend code. Run it in
the exact digest-pinned, network-disabled runtime image with read-only artifact
mounts. TensorRT-LLM inspection also requires the target GPU. The provider docs
...
installed InvarLock runtime therefore cannot substitute for a
manifest candidate. Model-runtime dependencies such as Torch, Transformers, or
...
`python -m invarlock.runtime_providers.tensorrt_llm_canary`, which checks the authenticated
...
provider checks. Run `runtime-qualification-canary` at the repository root for a small representative request
...
source, and executes the core provider code through an isolated
...
The provider documentation is the normative operator example for those bounds.
...
- [Python API](api-guide.md) identifies the provider ABI types exported through
  the stable facade.
- [Release verification](release-verification.md) explains package version and
  conformance checks.
