...
## Released public contract bundle

Tagged releases also publish
`invarlock-<version>-public-contract-bundle.tar.gz`, a standalone archive for
downstream consumers that do not want to clone the repository.

The bundle is generated by
`scripts/release/make_public_contract_bundle.py` and packages:
...
file in the archive. The bundle is reproducible for identical inputs: archive
timestamps are fixed and the manifest does not embed build-clock fields. Use it
as the canonical reference when verifying the bundle contents after download.

Example:

```bash
tar -xzf invarlock-<version>-public-contract-bundle.tar.gz
jq '.inventory[] | .path + " " + .sha256' \
  invarlock-<version>-public-contract-bundle/public_contract_bundle_manifest.json
```
