...
- **GuardChain** behavior: invariant checks plus spectral, RMT, and variance
  measurements that flag policy-defined candidate structural, edge-risk, and
  variance findings for review or enforcement. The thresholds are operational
  policy settings whose calibration limits are documented with each guard.
...
| Spectral family caps and the configured BH/Bonferroni-named candidate selector are applied as documented. | `docs/assurance/05-spectral-fpr-derivation.md` | Property and decision-parity tests exercise candidate selection, family caps, cap budgets, and authority-aware acceptance behavior. | `spectral.family_caps[*].kappa`, `spectral.families[*].kappa`, `spectral.multiple_testing`, `resolved_policy.guard_authority.spectral` | In exploratory local-baseline mode, selected violations may remain within `max_caps`; in strict reports, a complete selected finding blocks under `enforce` and remains observed under `observe`. Formal FDR/FWER control and Gaussian null calibration are **not established** by the repository. |
| RMT ε‑rule evaluates the declared **acceptance band** on activation edge‑risk growth. | `docs/assurance/06-rmt-epsilon-rule.md` | `tests/eval/test_assurance_contracts.py::test_rmt_epsilon_rule_acceptance_band`. | `rmt.{edge_risk_by_family_base,edge_risk_by_family,epsilon_default,epsilon_by_family,epsilon_violations,stable,status}`, `rmt.families.*.{edge_base,edge_cur,delta}`, `resolved_policy.guard_authority.rmt` | Complete ε-band findings block under `enforce` and remain observed under `observe`; missing, unsupported, or degraded evidence blocks in either mode. ε is an operational default, not a demonstrated cross-family/device false-alarm threshold. |
| Variance Equalization (VE) **enables only** when the predictive paired ΔlogNLL CI and mean are negative and also meet −`min_effect_lognll`, using the tier-specific interval sidedness. | `docs/assurance/07-ve-gate-power.md` | Report verifier validates enabled-VE predictive A/B provenance and CI; see `tests/eval/test_assurance_contracts.py::test_predictive_gate_respects_min_effect` and `tests/reporting/contracts/test_reporting_variance_enablement.py::test_validate_variance_enablement_rejects_missing_gate_provenance`. | `variance.{enabled,predictive_gate,ab_test,scope,proposed_scales}`, `resolved_policy.variance.{min_effect_lognll,predictive_one_sided}`, `resolved_policy.guard_authority.variance` | With zero min-effect, both the upper bound and mean must be strictly negative. A complete failing predictive-gate outcome blocks under `enforce` and remains observed under `observe`; incomplete or degraded evidence always blocks. Shipped values are operational defaults, not demonstrated power calibration. |
