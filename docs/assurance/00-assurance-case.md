...
| Guard Overhead stays within budget (≤ +1.0% PM when evaluated). | `docs/assurance/10-guard-overhead-method.md` | report gate `validation.guard_overhead_acceptable`; release verification additionally requires evaluated `guard_overhead` unless explicitly skipped. Bare vs guarded measurements use the same windows/seeds with a single toggle and snapshot/restore. | `guard_overhead.{bare_ppl,guarded_ppl,overhead_ratio,overhead_percent,overhead_threshold,evaluated,skipped}`, `validation.guard_overhead_acceptable` | Same schedule and seeds; bare control is guard-free; snapshot/restore or deterministic reload. Tiny runs may report `evaluated=false` and soft-pass at report build time, but release verification treats missing/unevaluated overhead as a release-blocking evidence gap unless skipped. |
