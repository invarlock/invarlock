...
| Variance Equalization (VE) **enables only** when the **predictive** paired ΔlogNLL CI excludes 0 (tier‑specific sidedness) **and** effect ≥ `min_effect_lognll`. | `docs/assurance/07-ve-gate-power.md` | Certificate validates predictive A/B provenance & CI; see `tests/eval/test_assurance_contracts.py::test_predictive_gate_respects_min_effect`. | `variance.{enabled,predictive_gate,ab_test,scope,proposed_scales}`, `resolved_policy.variance.{min_effect_lognll,predictive_one_sided}` | Balanced = **one‑sided** improvement; Conservative = **two‑sided**. Calibrated on same windows. |
