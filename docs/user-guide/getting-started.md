...
interpreter that installed the wheel. `make install-smoke` automates
the isolated local-build checks, including captured evaluation/recipient commands
before optional backend dependencies are installed. Never regenerate retained evidence to
make a recipe pass.
