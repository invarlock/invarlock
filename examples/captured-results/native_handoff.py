"""Project an independently pinned native rehearsal into captured evaluation records.
        ...
        "metrics": [
            {
                ...
                "subject_minimum": settings["subject_minimum"],
            },
            {
                "name": "latency",
                "kind": "recorded",
                ...
                "subject_maximum": settings["latency_subject_maximum_ms"],
            },
        ],
    }
    ...
    with tempfile.TemporaryDirectory(prefix="invarlock-native-projection-") as temp:
        ...
        for kind, cases in protocol["cases"].items():
            ...
            for side, template in protocol["prompts"].items():
                ...
                role = "subject" if side == "candidate" else "baseline"
                pending[f"{kind}/{role}.json"] = make_run(
                    rows,
                    source=source,
                    run_id=stem,
                    artifact_digest=artifact_digest,
