#!/usr/bin/env python3
"""Materialize a tiny GPTQModel subject checkpoint and local eval fixture."""
...
"""
    ...
    summary: dict[str, object] = {
        "format_version": "gptqmodel-fixture-v1",
...
def _require_dependencies() -> tuple[Any, Any, Any, Any, Any, Any]:
    try:
        ...
        raise SystemExit(
            ...
            "environment, for example: python -m pip install 'invarlock[gptq]'"
        ) from exc
    return (
        torch,
        GPTQModel,
...
def _quantized_module_types(model: Any) -> list[str]:
    ...
    for _, module in named_modules():
        ...
        if "gptqmodel" in normalized or "quant" in normalized:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "GPTQModel, and write a local evaluation fixture."
        )
    )
    parser.add_argument(
        "--baseline-dir",
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Directory where the GPTQModel subject checkpoint will be written.",
    )
    parser.add_argument(
        "--fixture-dir",
        required=True,
    ...
    parser.add_argument("--max-position-embeddings", type=int, default=96)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    reloaded = gptq_model.load(str(args.subject_dir), device="cpu")
    quant_cfg = _quantization_config(reloaded)
    if not quant_cfg:
        raise SystemExit("GPTQModel subject did not expose a quantization config.")
    ...
    summary = {
        ...
        "external_edit_type": "gptqmodel_gptq_4bit",
        "toolchain": "gptqmodel",
        "toolchain_versions": {
    ...
    checkpoint_refs = {
        ...
        "lane_id": "tiny-llama-gptqmodel-gptq-4bit",
        "created_at": timestamp,
        "baseline": {
            "kind": "byoe_checkpoint_ref",
            "path": str(args.baseline_dir),
            "purpose": "Deterministic tiny HF baseline for the GPTQModel example.",
        },
        "subject": {
            "kind": "byoe_checkpoint_ref",
            "path": str(args.subject_dir),
            "edit_workflow": "External GPTQModel GPTQ quantization",
            "external_edit_type": "gptqmodel_gptq_4bit",
            "purpose": "GPTQModel-quantized subject checkpoint loaded by hf_gptq.",
        },
        "artifacts": {
