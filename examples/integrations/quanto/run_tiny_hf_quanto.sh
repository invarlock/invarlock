...
usage() {
  cat <<'USAGE'
Usage: run_tiny_hf_quanto.sh [options]

Prepare a tiny HF causal checkpoint, then compare the normal HF baseline
against the same checkpoint loaded through InvarLock's hf_quanto adapter.

Options:
  --model-dir DIR              Output directory for the generated HF checkpoint.
                               Default: examples/integrations/quanto/models/tiny-llama-hf-quanto-baseline
...
if ! "$PYTHON_BIN" -c 'import optimum.quanto' >/dev/null 2>&1; then
  cat >&2 <<'MSG'
Missing example dependency: optimum-quanto

Install Quanto in the environment used for this example, for example:
  python -m pip install "invarlock[quanto]"

The core InvarLock install intentionally does not require Quanto.
MSG
  exit 2
fi

integration_preflight_host_cuda_device "$PYTHON_BIN" "$effective_execution_mode" "$effective_device" "Quanto" || exit $?

export PYTHONPATH="$REPO_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"

prepare_cmd=(
  "$PYTHON_BIN"
  "$SCRIPT_DIR/prepare_tiny_hf_quanto_fixture.py"
  --model-dir "$model_dir"
...
integration_log_step "prepare tiny HF checkpoint and Quanto fixture"
"${prepare_cmd[@]}"
...
compare_cmd=(
  ...
  --assurance "$assurance"
  --edit-label quanto_runtime_quantization
)
