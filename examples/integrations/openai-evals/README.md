...
score. This is an example-owned container bridge using locally hosted model
weights, not a hosted OpenAI model call or an installed evaluator provider.
The repository retains generic scorer qualification for OpenAI Evals but no
signed transaction for this profile.
...
evidence/verifier/builder keys. No OpenAI API key is needed for this local
completion adapter. Initial model and image downloads need network access.

```bash
make example-openai-evals EXAMPLE_ARGS="\
  ...
  --trust-root /secure/trust/openai-evals"
```

Append `--workspace PATH` to the full `EXAMPLE_ARGS` value when you want to retain
the transaction at a specific new path. The evidence and verifier keys must be caller-owned and
...
The command prints paths to the evidence, separate signed verification receipt
and HTML report. Check both integrity and policy in the verification result.
The launcher normally treats a policy rejection as failure; add
`--allow-policy-fail` only when you want to retain an authenticated rejection for
inspection. It does not change the policy result or permit corrupt evidence.
...
interval width to 20 percentage points, requires its lower bound to be at least
−20 percentage points, and requires each model to solve at least 20% of the corpus.
