...
usage() {
  cat <<'USAGE'
Usage: run_tiny_magnitude_prune.sh [options]

Materialize a tiny magnitude-pruned subject checkpoint, then compare it
against the baseline with InvarLock's shared integration wrapper.

Options:
  --baseline VALUE             Baseline model ID or local path.
                               Default: sshleifer/tiny-gpt2
  --subject-dir DIR            Output directory for the pruned subject.
                               Default: examples/integrations/magnitude_prune/models/tiny-gpt2-magnitude-pruned
  --fixture-dir DIR            Generated local JSONL/preset directory.
                               Default: examples/integrations/magnitude_prune/artifacts/tiny-magnitude-prune-fixture
  --report-out DIR             Output directory for InvarLock artifacts.
                               Default: examples/integrations/magnitude_prune/reports/tiny-magnitude-prune/<artifact-lane>
  ...
  --prune-fraction VALUE       Fraction of each eligible tensor to zero.
                               Default: 0.0025
  --allow-network              Allow model/dataset downloads.
  --force                      Replace an existing subject directory.
  --materialize-only           Stop after writing the pruned checkpoint.
...
subject_dir="$SCRIPT_DIR/models/tiny-gpt2-magnitude-pruned"
fixture_dir="$SCRIPT_DIR/artifacts/tiny-magnitude-prune-fixture"
report_out="$SCRIPT_DIR/reports/tiny-magnitude-prune"
...
if [[ -z "$PYTHON_BIN" ]]; then
  select_python_bin transformers
fi
...
integration_log_header "Magnitude-prune integration example"
...
if ! "$PYTHON_BIN" -c 'import transformers, torch' >/dev/null 2>&1; then
  cat >&2 <<'MSG'
Missing example dependency: transformers and torch

Install the Hugging Face stack in the environment used for this example, for example:
...
integration_preflight_host_cuda_device "$PYTHON_BIN" "$effective_execution_mode" "$effective_device" "magnitude-prune" || exit $?

export PYTHONPATH="$REPO_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"

materialize_cmd=(
  "$PYTHON_BIN"
  "$SCRIPT_DIR/materialize_tiny_magnitude_prune_subject.py"
  --baseline "$baseline"
  --output-dir "$subject_dir"
  --fixture-dir "$fixture_dir"
  --prune-fraction "$prune_fraction"
)
...
integration_log_step "materialize tiny magnitude-pruned subject and local fixture"
