...
Train and merge a tiny PEFT LoRA subject from an immutable profile, verify its
artifacts independently, then compare it with the pinned baseline.

Options:
  --training-profile ID       Immutable training profile. Defaults to the
                              canonical CPU or CUDA LoRA profile for the lane.
  ...
  --report-out DIR            Output directory for InvarLock artifacts. Default
                              ends in reports/tiny-peft-lora/<artifact-lane>.
...
The training receipt binds the immutable profile, data, baseline, adapter,
merged subject, and deltas. It does not independently attest execution history;
trusted execution or an independent rerun is still required.
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --target-module|--rank|--alpha|--lora-init-scale)
      echo "$1 is incompatible with immutable training profiles; define and review a new profile instead." >&2
      exit 2
      ;;
    -h|--help) usage; exit 0 ;;
...
select_python_bin() {
  ...
  for candidate in python "$REPO_ROOT/.venv/bin/python" python3; do
    ...
    if "$candidate" -c 'import peft, torch, transformers' >/dev/null 2>&1; then
...
if ! python_bin_is_executable "$PYTHON_BIN" || ! "$PYTHON_BIN" -c 'import peft, torch, transformers' >/dev/null 2>&1; then
...
if [[ -z "$training_profile" ]]; then
  training_profile="$(integration_default_training_profile lora_merge "$effective_device")"
fi
integration_load_training_profile "$PYTHON_BIN" "$PROFILES_PATH" "$training_profile" lora_merge || exit $?
integration_preflight_training_device "$PYTHON_BIN" "$TRAINING_DEVICE" "PEFT LoRA" || exit $?
...
integration_log_step "execute and independently verify immutable LoRA training profile"
...
integration_run_source_archive_clean \
  ...
  --model-id "$baseline" \
  --format-version peft-lora-fixture-v1
...
integration_stage_training_evidence \
  "$PYTHON_BIN" "$REPO_ROOT" "$PROFILES_PATH" "$training_profile" \
  "$subject_dir" "$report_out" "$allow_network" "attn"
