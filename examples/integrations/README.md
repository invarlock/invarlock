...
| Hugging Face Optimum | `compatibility-investigation` via `optimum_onnx_export/` | ONNX deployment artifact compatibility report |
...
- `optimum_onnx_export/` exports a tiny model with Optimum ONNX and records why
  the ONNX Runtime artifact is sidecar deployment evidence rather than a direct
  input to the shared HF checkpoint comparison wrapper.
