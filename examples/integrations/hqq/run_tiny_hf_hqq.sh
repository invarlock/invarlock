...
usage() {
  cat <<'USAGE'
Usage: run_tiny_hf_hqq.sh [options]

Prepare a tiny HF causal checkpoint, then compare the normal HF baseline
against the same checkpoint loaded through InvarLock's hf_hqq adapter.

Options:
  --model-dir DIR              Output directory for the generated HF checkpoint.
                               Default: examples/integrations/hqq/models/tiny-llama-hf-hqq-baseline
  --fixture-dir DIR            Generated local JSONL/preset directory.
                               Default: examples/integrations/hqq/artifacts/tiny-hf-hqq
  --report-out DIR             Output directory for InvarLock artifacts.
                               Default: examples/integrations/hqq/reports/tiny-hf-hqq/<artifact-lane>
...
model_dir="$SCRIPT_DIR/models/tiny-llama-hf-hqq-baseline"
fixture_dir="$SCRIPT_DIR/artifacts/tiny-hf-hqq"
report_out="$SCRIPT_DIR/reports/tiny-hf-hqq"
...
integration_log_header "HQQ integration example"
...
if ! "$PYTHON_BIN" -c 'import hqq' >/dev/null 2>&1; then
  cat >&2 <<'MSG'
Missing example dependency: hqq

Install HQQ in the environment used for this example, for example:
  python -m pip install "invarlock[hqq]"

The core InvarLock install intentionally does not require HQQ.
MSG
  exit 2
fi

integration_preflight_host_cuda_device "$PYTHON_BIN" "$effective_execution_mode" "$effective_device" "HQQ" || exit $?

export PYTHONPATH="$REPO_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"

prepare_cmd=(
  "$PYTHON_BIN"
  "$SCRIPT_DIR/prepare_tiny_hf_hqq_fixture.py"
  --model-dir "$model_dir"
...
integration_log_step "prepare tiny HF checkpoint and HQQ fixture"
"${prepare_cmd[@]}"
...
compare_cmd=(
  ...
  --assurance "$assurance"
  --edit-label hqq_runtime_quantization
)
